...
{ --------------------------------------------------------------- }
{ === Datenbank ==================================================== }
{ $Id: xp2db.pas,v 1.19 2002/01/05 09:15:23 mm Exp $ }
...
uses  xpglobal,crt,dos,typeform,fileio,inout,keys,datadef,datadef1,
      database,databaso,resource,maus2,
{$IFDEF USEINDEXCACHE }
  {$IFDEF BP }
      ems,
  {$ENDIF }
...
begin
 ...
 {mb_prioflags  := dbGetFeldNr(mbase,'prioflags');}
...
(*  procedure kk;
  ...
  begin
    ...
    while not dbEOF(mbase) do begin
      ...
      if (nt=nt_Fido) or (nt=nt_Magic) or (nt=nt_ZConnect) or
  ...
  { Feld 'PrioFlags' in Nachrichtendatei einfÅgen (ab 3.3) }
 {procedure NewFieldMsgPrioFlags;
  var fld : dbFeldTyp;
  begin
    with fld do begin
      fname:='prioflags'; ftyp:=dbTypeInt;
      fsize:=4; fnlen:=10;
      end;
    dbAppendField(MsgFile,fld);
  end;}
...
{$endif}
  ...
  if not exist(MsgFile+dbext) then begin   { XPOINT: Nachrichtendatei }
    ...
    if not dbHasField(msgFile,'mimetyp') then
      NewFieldMsgMimetyp;
    {if not dbHasField(msgFile,'prioflags') then
      NewFieldMsgPrioFlags;}
    end;
  ...
  while not dbEOF(dd) do begin
    if (not registriert.uucp and (dbReadInt(dd,'netztyp')=nt_UUCP)) or
      { (not registriert.non_uucp and (dbReadInt(dd,'netztyp')<>nt_UUCP)) then }
       (not registriert.non_uucp and not (dbReadInt(dd,'netztyp') in [nt_UUCP,nt_PPP])) or
       (not registriert.ppp and (dbReadInt(dd,'netztyp')=nt_PPP)) then
      reg_hinweis:=true;
...
{
  $Log: xp2db.pas,v $
  Revision 1.19  2002/01/05 09:15:23  mm
  - dbSetIndexCache auf maxcache umgestellt
  - Speicherueberpruefung unter 32Bit ueberfluessig

  Revision 1.18  2001/12/30 11:00:32  mm
  - Fix: Indexcache nun auch in 32Bit-Version einschaltbar

  Revision 1.17  2001/06/18 20:17:26  oh
  Teames -> Teams

  Revision 1.16  2001/02/09 22:14:26  rb
  define fÅr Indexcache

  Revision 1.15  2000/10/26 21:15:53  rb
  HD: Fix fÅr Division by Zero

  Revision 1.14  2000/10/10 07:54:08  rb
  Supersedes-Support fertiggestellt
  - Autoversand
  - SV: Einzelnachrichten

  Revision 1.13  2000/08/08 08:59:31  MH
  Reg-Hinweis-Anzeige wieder hergestellt

  Revision 1.12  2000/08/07 19:46:46  MH
  Loginfo oben hinzugefÅgt

  Revision 1.11  2000/06/24 16:46:20  tg
  hart codiertes Update auf v3.2 auf Verstr geaendert

  Revision 1.10  2000/06/06 07:59:42  MH
  RFC/PPP: Weitere Anpassungen und Fixes

  Revision 1.8  2000/04/30 11:34:42  MH
  Altes Datenbankformat wieder hergestellt

  Revision 1.5  2000/04/10 22:13:08  rb
  Code aufgerÑumt

  Revision 1.4  2000/04/09 18:22:31  openxp
