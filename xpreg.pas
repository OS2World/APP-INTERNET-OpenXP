...
{ --------------------------------------------------------------- }
{ $Id: xpreg.pas,v 1.10 2001/06/23 10:37:57 MH Exp $ }
...
uses  xpglobal, crt,dos,typeform,fileio,inout,keys,winxp,montage,
      video,datadef,database,maus2,maske,clip,resource,printerx,
      xp0,xp1,xp1o,xp1o2,xp1input,xpnt;
...
var x,y,i : byte;
  ...
  var x,y  : byte;
      ...
      z    : taste;
  begin
    if registriert.uucp or registriert.non_uucp then begin
      pushhp(1553);
...
begin
  ...
  {$IFDEF EXTERN}
    if deutsch then
      s:=x_copyright + ' ' + author_name + ' - basierend auf XP2-Version'
    else
      s:=x_copyright + ' ' + author_name + ' - based of XP2-Version';
  {$ELSE}
...
begin
  {$IFDEF EXTERN}
    if deutsch then begin
      i:=15;
      msglines:=ival(getres2(530,i))+i;
      s:=x_copyright + ' ' + author_name + ' - basierend auf XP2-Version';
    end else begin
      i:=0;
      msglines:=ival(getres2(530,i));
      s:=x_copyright + ' ' + author_name + ' - based of XP2-Version' ;
    end;
  {$ELSE}
    i:=0;
  ...
  for i:=i+1 to msglines do
  begin
    s:=getres2(530, i);  inc(y);
    wrt(x+3, y+3, s);
  end;
  mon;
  pushhp(1550);
  quit:=(ReadButton(x+45,y+5,2,'*'+getres2(530,30),1,true,z) <> 1);
  pophp;
  closebox;
  freeres;
end;
...
{
  $Log: xpreg.pas,v $
  Revision 1.10  2001/06/23 10:37:57  MH
  - Betascreen fÅr externe Kompilate angepasst

  Revision 1.9  2000/12/30 18:55:23  MH
  Fix:
  - Betascreen bei Paramter '/t:' ignorieren
  - Betaschalter fÅr BetaScreen hinzugefÅgt

  Revision 1.8  2000/11/11 19:33:44  MH
  Lîschen von allen Reg-Keys ermîglichen

  Revision 1.7  2000/07/26 21:33:47  MH
  LIZENZ.DOC wurde im DOC-Verzeichnis gesucht...grrrr

  Revision 1.6  2000/06/08 20:07:27  MH
  Teamname geandert

  Revision 1.5  2000/04/10 22:13:17  rb
  Code aufgerÑumt

  Revision 1.4  2000/04/09 18:32:53  openxp
