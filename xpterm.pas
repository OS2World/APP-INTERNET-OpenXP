...
{ --------------------------------------------------------------- }
{ $Id: xpterm.pas,v 1.7 2001/12/26 20:20:38 MH Exp $ }
...
uses  xpglobal, crt,dos,typeform,fileio,inout,keys,uart,datadef,database,maus2,
      video,
{$IFDEF CAPI }
      capi,
{$ENDIF }
      resource,xp0,xp1,xp1o2,xp1input;
...
begin
  if comn[comnr].fossil and not FOSSILdetect then begin
    trfehler(732,errortimeout);   { 'Kein FOSSIL-Treiber installiert - verwende eingebauten Treiber' }
...
{$ENDIF }
    ...
    if direct then begin
      ...
      if (OStype<>OS_2) and rest then begin
...
{
  $Log: xpterm.pas,v $
  Revision 1.7  2001/12/26 20:20:38  MH
  - SetUart hinzugefÅgt

  Revision 1.6  2001/06/18 20:17:44  oh
  Teames -> Teams

  Revision 1.5  2001/04/04 19:57:03  oh
  -Timeouts konfigurierbar

  Revision 1.4  2001/02/26 22:21:09  MH
  - Fixversuch (JG) gegen Probleme bei langsamen Modems

  Revision 1.3  2000/05/25 23:31:13  rb
