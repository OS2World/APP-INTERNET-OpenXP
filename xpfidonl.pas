...
{ --------------------------------------------------------------- }
{ $Id: xpfidonl.pas,v 1.7 2001/04/04 19:57:03 oh Exp $ }
...
uses  xpglobal, dos,typeform,montage,fileio,maske,resource,archive,
      xp0,xp1,xp1o,xp3;
...
(*
  if (nodelist<>'') and not exist(FidoDir+nodelist) then begin
    trfehler1(214,nodelist,errortimeout_short);   { 'Nodelist %s fehlt' }
    nodelist:='';
    end;
  if (pointlist<>'') and not exist(FidoDir+pointlist) then begin
    trfehler1(215,pointlist,errortimeout_short);   { 'Pointlist %s fehlt' }
    pointlist:='';
    end;
*)
end;
...
begin
  ...
  for i:=1 to NL_anz do
    if not exist(FidoDir+NLfilename(i)) then
      trfehler1(214,FidoDir+NLfilename(i),errortimeout_short);  { 'Node-/Pointliste %s fehlt!' }
  if NL_anz>0 then begin
    NL_Datecheck;
...
var diffdir  : pathstr;
  ...
  begin
    ...
    filemode:=fm;
    if s1<>s2 then begin
      trfehler1(2111,NLfilename(i),errortimeout);   { '%s: Falsche Version von Nodelist oder Nodediff' }
...
end.
{
  ...
  Revision 1.6  2000/06/08 20:06:41  MH
  Teamname geandert

  Revision 1.5  2000/04/10 22:13:15  rb
  Code aufgerÑumt

  Revision 1.4  2000/04/09 18:30:39  openxp
