...
begin
  ...
  maddbool(3,11,getres2(250,14),newbrettende);mhnr(219);          { 'neue Bretter am Ende anhÑngen'        }
  ...
  maddbool(3,15,getres2(250,21),OtherQuoteChars);                 { 'Farbe auch fÅr Quotezeichen : und |'  }
...
begin
  ...
  freeres;       { AutoDatumsBezuege wird nicht mehr benîtigt! }
  readmask(brk);
  if not brk and mmodified then
    GlobalModified;
  enddialog;
...
begin
  ...
  maddbool(3,10,getres2(259,18),longbetr);     { 'Betreff verlÑngern' }
...
begin
  ...
  while not (p=0) do begin
    if pos('\',s1)>9 then begin
      rfehler(221); { 'Mehr als acht Zeichen sind fÅr Verzeichnisse nicht zulÑssig!' }
      formpath:=false;
      exit;
    end;
    p:=pos('\',s1);
...
begin
  ...
  maddbool(3,8,getres2(271,9),PGP_log);        { 'Logfile fÅr automatische Aktionen' }
...
(*
  maddbool(3,13,getres2(271,12),PGP_Fido);       { 'PGP auch fÅr Fido verwenden' }
  maddbool(3,12,getres2(271,11),PGP_RFC{UUCP});{ 'PGP auch fÅr RFC verwenden' }
  mset1func(testxpgp);
*)
  readmask(brk);
  if not brk and mmodified then
...

{
  ...
  MK: Weiterschalter nach énderung im BedienungsmenÅ sofort wirksam
  MH: Das selbe gilt auch fÅr den Extrakttyp
  ...
  - Feldtausch MenÅ Åbersichtlicher aufgebaut
