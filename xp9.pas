...
{ --------------------------------------------------------------- }
{ $Id: xp9.pas,v 1.28 2001/09/13 07:17:29 MH Exp $ }
...
{ function  TestPPPservers(var s:string):boolean; s. MultimenÅ }
function  uuz_in_test(var s:string):boolean;
function  uuz_out_test(var s:string):boolean;
...
uses xp2,xp3,xp3o,xp9bp,xp10,xpnt,xpterm,xp9ppp;

const umtyp : array[0..5] of string[5] =
              ('IBM','ASCII','ISO','Tab.1','Tab.2','Tab.3');

      enetztypen = 12;
      ntnr   : array[0..enetztypen-1] of byte = (2,0,40,41,20,30,31,3,4,10,11,90);
    { ntypes : array[0..enetztypen-1] of string[10] = ('Z-Netz','ZConnect',
                 'RFC/UUCP','RFC/PPP','MausTausch','Fido','QWK','MagicNET',
                 'ProNET','QuickMail','GS-Mailbox','Turbo-Box'); }
...
begin
  if s='' then testvertreterbox:=true
    ...
    if not dbFound then begin
      ...
      if (DomainNt=nt_PPP) or (DomainNt=nt_UUCP) then
        ok:=(nt=nt_PPP) or (nt=nt_UUCP) else
        ok:=(DomainNt=nt)
...
(*
function TestPPPservers(var s:string):boolean;
var ok : boolean;
    p  : byte;
    s2 : string;
begin
  ...
  if s<>'' then begin
    ...
    repeat
      p:=blankpos(s2);
      if p=0 then p:=length(s2)+1;
      if ntBoxNetztyp(left(s2,p-1))<>nt_PPP then begin
        rfehler1(933,left(s2,p-1));  { '%s ist keine eingetragene PPP-Serverbox!' }
        ok:=false;
      end;
      s2:=trim(mid(s2,p+1));
    until s2='';
...
begin
  ptest:=progtest(s);
  if (s<>'') and ((pos('$PUFFER',s)=0) or (pos('$SPOOL',s)=0)
  or ((pos('$PUFFER',s)<pos('$SPOOL',s)) and (ustr(left(s,3))='UUZ')))
  then begin
    rfehler(934);
    { 'Der UUZ-Aufruf muss $PUFFER und $SPOOL in der richtigen Reihenfolge enthalten' }
    uuz_in_test:=false;
  end
  else uuz_in_test:=ptest;
end;
...
begin
  ptest:=progtest(s);
  if (s<>'') and ((pos('$PUFFER',s)=0) or (pos('$SPOOL',s)=0)
  or ((pos('$SPOOL',s)<pos('$PUFFER',s)) and (ustr(left(s,3))='UUZ')))
  then begin
    rfehler(934);
    { 'Der UUZ-Aufruf muss $PUFFER und $SPOOL in der richtigen Reihenfolge enthalten' }
    uuz_out_test:=false;
  end
  else uuz_out_test:=ptest;
end;
...
var d         : DB;
 ...
 {  case flags and 6 of
  ...
  begin
    ...
    maddstring(3,2,getres2(903,3),name,BoxRealLen,BoxNameLen,'>'); mhnr(461); { 'Systemname ' }
  ...
  begin
    ...
    if not brk then begin
      ...
      if nt=nt_PPP then begin
        rfehler(832); {'Diese Funktion wird von RFC/PPP-Boxen nicht unterstÅtzt'}
        exit;
      end;
...
procedure gf_getntyp(var s:string);
var uucp,ppp : boolean;
begin
  gf_fido:=(lstr(s)=lstr(ntName(nt_Fido)));
  uucp:=(lstr(s)=lstr(ntName(nt_UUCP)));
  ppp:=(lstr(s)=lstr(ntName(nt_PPP)));
  if (lstr(s)=lstr(ntName(nt_Maus))) or gf_fido or uucp or ppp then
    set_chml(userfield,'')
  else
    set_chml(userfield,'>');
  if uucp or ppp then
...
begin
  ...
  maddtext(3,2,getres2(911,6),col.coldiahigh);    { 'Bitte beachten Sie, das bei RFC/PPP-Boxen' }
  maddtext(3,3,getres2(911,7),col.coldiahigh);    { 'externe Clients benîtigt werden.' }
  maddtext(3,5,getres2(911,1),col.coldiahigh);    { 'Bitte geben Sie den Namen Ihrer Stammbox, den' }
  maddtext(3,6,getres2(911,2),col.coldiahigh);    { 'Netztyp der Box und Ihren Usernamen ein:' }
  name:=''; user:='';
  ntyp:=ntName(nt_PPP); nt:=nt_PPP;
  if not (nt in ntAllowed) then begin    { unreg. Turbobox-Version }
    ntyp:='Turbo-Box'; nt:=nt_Turbo;
  end;
  maddstring(3,8,getres2(911,3),ntyp,20,20,''); mhnr(681);   { 'Netztyp   ' }
  for i:=0 to enetztypen-1 do
    if (ntnr[i] in ntAllowed) then
      mappsel(true,ntName(ntnr[i]));
  mset3proc(gf_getntyp);
  maddstring(3,10,getres2(911,4),name,20,20,'>-_0123456789:/.'+range('A','Z')+'éôö');
    mhnr(680);                                       { 'Boxname   ' }
  DomainNt:=-1;
  msetvfunc(xp9_testbox);
  maddstring(3,12,getres2(911,5),user,30,30,'>'); mhnr(682);   { 'Username  ' }
  userfield:=fieldpos;
  msetvfunc(notempty2);
  ...
  SaveConfig2;
  if (nt=nt_UUCP) or (nt=nt_PPP) then begin
    XP_ID_AMs:=false;
    SaveConfig;
  end;
end;
...
{
  $Log: xp9.pas,v $
  Revision 1.28  2001/09/13 07:17:29  MH
  -.-

  Revision 1.27  2001/06/18 20:17:40  oh
  Teames -> Teams

  Revision 1.26  2000/11/18 19:23:54  MH
  Systeme:
  - Server kann aus Boxenliste ausgewÑhlt werden
  - Filelisten: auch bei Servern > 8 Zeichen erkennen

  Revision 1.25  2000/11/09 21:45:13  MH
  Fix: Server > 15 Zeichen

  Revision 1.24  2000/10/12 00:03:16  MH
  RFC/PPP -> Fix: Alias verbieten...

  Revision 1.23  2000/07/27 09:31:59  MH
  RFC/PPP: Fix: Edit/Systeme:
  Es wurde die falsche Datenbank zu frÅh geschlossen

  Revision 1.22  2000/07/14 18:04:00  MH
  RFC/PPP: Bei erst Installation Hinweis Åber Verwendung
           benîtigter externer Clients anzeigen

  Revision 1.21  2000/07/12 23:56:16  MH
  DefaultBox bei Erstinstallation ist nun ein RFC/PPP-Typ

  Revision 1.20  2000/07/04 08:58:50  MH
  RFC/PPP: Meldung in Resource aufgenommen

  Revision 1.19  2000/07/03 12:14:51  MH
  RFC/PPP: Teile des RFC/PPP-BoxmenÅs in XP9PPP.PAS ausgelagert
  - MenÅ Namen: Oben wird nun die vollstÑndige E-Mail-Adresse
    angezeigt

  Revision 1.18  2000/06/30 20:16:49  rb
  Bugfix fÅr UUZ-ParameterprÅfung bei PPP

  Revision 1.17  2000/06/30 19:12:46  MH
  RFC/PPP: ClientmenÅ:
  - UUZ-Aufrufe werden auf korekte Angabe (Reihenfolge/Vorhandensein)
    der Makros ÅberprÅft

  Revision 1.16  2000/06/22 03:47:09  rb
  Haltezeit-Bug bei ver32 gefixt

  Revision 1.15  2000/06/21 07:02:48  MH
  RFC/PPP: Pakete mitsenden implementiert

  Revision 1.14  2000/06/17 09:40:36  MH
  RFC/PPP: Pakete mitsenden ins Menue aufgenommen

  Revision 1.13  2000/06/12 11:53:15  MH
  RFC/PPP: Fix: AM-Vertretersystem

  Revision 1.12  2000/06/12 07:15:37  MH
  CMD kann nun auch als Programm eingetragen werden

  Revision 1.11  2000/06/10 14:17:51  MH
  RFC/PPP: Vertretersysteme auch AM moeglich

  Revision 1.10  2000/06/10 14:13:42  MH
  RFC/PPP: Vertretersysteme nun moeglich

  Revision 1.9  2000/06/05 19:56:27  MH
  RFC/PPP: Auswahl als Fileserver (Systeme) verhindern

  Revision 1.8  2000/06/04 04:31:21  MH
  RFC/PPP: Code aufgeraeumt

  Revision 1.7  2000/05/17 19:13:26  MH
  Neuer Boxentyp: RFC/PPP

  Revision 1.5  2000/04/10 22:13:13  rb
  Code aufgerÑumt

  Revision 1.4  2000/04/09 18:28:40  openxp
