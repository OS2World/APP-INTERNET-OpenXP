...
{ --------------------------------------------------------------- }
{ $Id: xp10.pas,v 1.16 2002/01/05 22:06:13 MH Exp $ }
...
uses xpglobal,crt,dos,stack,typeform,fileio,inout,keys,montage,feiertag,winxp,
     maske,datadef,database,maus2,lister,resource,
...
var brk      : boolean;
    ...
    movefrom : integer;
    oldft    : longint;
    blist    : array[0..maxboxen] of string;
  ...
  var anz  : integer;
      ...
      d    : DB;
  begin
    if anzahl=maxboxen then begin
      rfehler1(1007,strs(maxboxen));    { 'Maximal %s Zeilen mîglich!' }
      exit;
    end;
    dbOpen(d,BoxenFile,1);
    anz:=0;
    repeat
      if (dbReadInt(d,'netztyp')=41) and
         (BoxPar^.BoxName<>dbReadStr(d,'BoxName')) then
        inc(anz);
      dbNext(d);
    until dbEOF(d);
    if anz=0 then fehler('Keine RFC/PPP-Box zum hinzufÅgen vorhanden!')
    else begin
      listbox(35,min(anz,screenlines-5),'Box hinzuf. (Pkt. mitsenden)');
      dbGoTop(d);
      repeat
        if (dbReadInt(d,'netztyp')=41) and
           (BoxPar^.BoxName<>dbReadStr(d,'BoxName')) then
          app_l(' '+dbReadStr(d,'BoxName'));
        dbNext(d);
      until dbEOF(d);
      list(brk);
      closebox;
      if not brk then begin
        box:=trim(get_selection);
        i:=anzahl;
        while (i>0) and (box<>blist[i]) do { Eintrag schon vorhanden? }
          dec(i);
        if i>0 then errsound else
        begin
          inc(anzahl);
          blist[0]:=blist[a+p];  { Ziel = (a+p) }
          blist[a+p]:=box;
          for i:=(a+p+1) to anzahl do begin
            box:=blist[i];
            blist[i]:=blist[0];
            blist[0]:=box;
          end;
  ...
  procedure MoveBoxLine;
  var
    s : string[BoxNameLen];
    i : integer;
  begin                           { Ziel = (a+p); Quelle = movefrom }
    blist[0]:=blist[a+p];
    blist[a+p]:=blist[movefrom];
    if movefrom<a+p then
  ...
  begin
    ...
    if ReadJN(getreps2(1018,5,s),true) then begin   { 'Zeile "%s" lîschen' }
      if a+p<anzahl then begin  { a+p = Ziel }
        for i:=(a+p) to anzahl-1 do
          blist[i]:=blist[i+1];
        blist[i+1]:='';
      end
      else
  ...
  (*  mAx: wird fÅr den neuen Standard nicht mehr benîtigt, trotzdem bitte
           stehen lassen...war ein Haufen arbeit...;)
  procedure MultiClient;
  var
    d        : db;
    i,i1     : integer;
    spos     : integer;
    ...
    procedure ClientMsgBox(txt:string);
    begin
      y:=3; x:=(screenwidth-75) div 2 +1;
      blindon(true);
      wpushs(x,x+75-1,y,y+4-1,'-');
      attrtxt(col.colmboxrahmen);
      forcecolor:=true;
      case getrahmen of
        1 : rahmen1(x,x+75-1,y,y+4-1,'');
        2 : rahmen2(x,x+75-1,y,y+4-1,'');
      end;
      forcecolor:=false;
      mwrt(x+2,y,' '+txt+' ');
      attrtxt(col.colmbox);
      clwin(x+1,x+75-2,y+1,y+4-2);
      mwrt(x+2,y+1,'Mail: '+left(boxpar^.ppp_mail,65));
      mwrt(x+2,y+2,'News: '+left(boxpar^.ppp_news,65));
    end;

    procedure cutmacro(client:string);
    var
      ic : integer;
    begin
      i:=0;
      for ic:=1 to length(client) do
        if client[ic]='$' then inc(i);
      if i>1 then begin
        i1:=pos('$',client);
        if lastchar(client)<>' ' then client:=client+' ';
        i:=rightpos('$',client);
        i:=posn(' ',client,i);
        clientpar:=copy(client,i1,i-i1);
        i:=pos('$CONFIG',ustr(clientpar));
        if i<>0 then
          clientpar:=copy(clientpar,i+7,length(clientpar)-i-6);
        clientpar:=copy(clientpar,pos('$',clientpar),length(clientpar));
        if firstchar(clientpar)<>' ' then clientpar:=' '+clientpar;
      end;
    end;

    procedure setcpar(client:string);
    begin
      if posn(' ',client,i)=0 then
        client:=client+' ';
      i:=posn(' ',client,i)-2;  { ende   }
      i1:=pos('/',client)-2;    { anfang }
      clientpar:=clientpar+copy(client,i1+1,i);
      clientpar:=rtrim(clientpar);
    end;

    function getclient(client:string):string;
    begin
      clientpar:='';
      spos:=pos('START.EXE /W',ustr(client));
      if spos=1 then client:=copy(client,14,length(client)-13);
      cutmacro(client);
      i:=rightpos('/',client);
      if i<>0 then
        setcpar(client);
      i1:=pos('$CONFIG',ustr(client));
      if i1<>0 then
        getclient:=iifs(spos=1,'START.EXE /W ','')+left(client,i1+6)+bflist+clientpar
      else
      if pos('.EXE',ustr(client))<>0 then
        getclient:=iifs(spos=1,'START.EXE /W ','')+left(client,pos('.EXE',ustr(client))+3)+bflist
      else
      if pos('.BAT',ustr(client))<>0 then
        getclient:=left(client,pos('.BAT',ustr(client))+3)+bflist;
    end;

  begin
    pushhp(911);
    ClientMsgBox('Client-Einstellungen (vorher)');
    case ReadIt(ival(getres2(1024,1)),getres2(1024,2),
      { 'Clienteinstellungen anpassen...' }
      getres2(1024,3),1,brk) of
      1 : ctyp:=1; { '~Nur ^Mail-Client , }
      2 : ctyp:=2; {   Nur ^News-Client , }
      3 : ctyp:=3; {   ^Beide Clients~    }
    end;
    closebox;
    pophp;
    if brk then exit;
    ...
    for i:=1 to anzahl do begin
      ...
      if dbFound then
        bflist:=bflist+dbReadStr(d,'DateiName')+'.BFG ';
    end;
    dbClose(d);
    dellast(bflist);
    with boxpar^ do begin
      if (ctyp in [1,3]) and (ppp_mail<>'') then
        ppp_mail:=getclient(ppp_mail);
      if (ctyp in [2,3]) and (ppp_news<>'') then
        ppp_news:=getclient(ppp_news);
    end;
    ClientMsgBox('Client-Einstellungen (nachher)');
    wait(curoff);
    closebox;
  end;
  *)

  procedure makebfglist;
  var
    d : db;
    i : integer;
  begin
    BoxPar^.PPP_BoxPakete:='';
    dbOpen(d,BoxenFile,1);
    for i:=1 to anzahl do begin
      dbSeek(d,boiname,ustr(blist[i]));
      BoxPar^.PPP_BoxPakete:=BoxPar^.PPP_BoxPakete+dbReadStr(d,'dateiname')+' ';
    end;
    dellast(BoxPar^.PPP_BoxPakete);
    dbClose(d);
  end;

  procedure makeboxlist;
  var
    d : db;
    i : integer;
  begin
    dbOpen(d,BoxenFile,1);
    for i:=1 to anzahl do begin
      dbSeek(d,boidatei,ustr(blist[i]));
      if dbFound then
        blist[i]:=dbReadStr(d,'boxname');
    end;
    dbClose(d);
  end;
...
begin
  case typ of
    ...
    7 : begin                       { Multiboxbetrieb }
          filewidth:=30;
          width:=ival(getres2(1018,1));
          buttons:=getres2(1018,2);   { ' ^EinfÅgen , ^Verschieben , ^Lîschen ,  ^OK  ' }
          okb:=4; edb:=0;
          pushhp(910);
          for ii:=0 to maxboxen do blist[ii]:='';
          with boxpar^do begin
            if (PPP_BoxPakete<>'') then anzahl:=1 else anzahl:=0;
            for ii:=1 to length(PPP_BoxPakete) do
              if cpos(PPP_BoxPakete[ii],' ')>0 then inc(anzahl);
            blist[0]:=PPP_BoxPakete;
            for ii:=1 to anzahl do begin
              p:=blankpos(blist[0]);
              if p=0 then p:=length(blist[0])+1;
              if p>=3 then begin
                blist[ii]:=left(blist[0],p-1);
  ...
  repeat
    ...
    if (typ=4) or (typ=7) then
      if t=keyins then nr:=1
      else if t=keydel then nr:=3;
    if (nr<>0) and (nr<>99) then bp:=abs(nr);
    { c:=UpCase(t[1]); }
    if ((typ=7) and (nr=1) and (eanzahl=maxboxen)) then
      rfehler1(1004,strs(maxboxen))     { 'Maximal %s EintrÑge mîglich!' }
    else
    ...
    if (typ=7) and (nr=okb) and modi then begin
      (* alte Fassung!
      BoxPar^.PPP_BoxPakete:='';
      for ii:=1 to anzahl do begin
        BoxPar^.PPP_BoxPakete:=BoxPar^.PPP_BoxPakete+blist[ii]+' ';
      end;
      dellast(BoxPar^.PPP_BoxPakete);
      *)
      makebfglist; { BFG-Dateinamen schreiben }
      {if ReadJN('Clienteinstellung anpassen',true) then MultiClient;}
      modi:=false;
    end;
...
{
  $Log: xp10.pas,v $
  Revision 1.16  2002/01/05 22:06:13  MH
  - ueberfluessigen Code auskommentiert

  Revision 1.15  2001/11/04 08:11:59  MH
  - Multi: Einf- und Entf-Taste berÅcksichtigen

  Revision 1.14  2001/08/20 13:04:13  MH
  ...
  Revision 1.13  2001/07/16 10:14:42  MH
  - XP3P-Paras angepasst

  Revision 1.12  2001/07/10 22:49:00  MH
  - Multi: Optimiert

  Revision 1.11  2001/07/10 14:59:43  MH
  - Muli: START.EXE /W integriert

  Revision 1.10  2001/07/10 11:36:45  MH
  - Muli: Nochmal abgesichert, damit Parameter sicher nicht verloren gehen

  Revision 1.9  2001/07/10 11:19:59  MH
  - Muli: auch '-' Parameter bleiben nun erhalten!

  Revision 1.8  2001/07/10 10:45:54  MH
  - Muli: Parameter bleiben nun erhalten!

  Revision 1.7  2001/07/09 01:10:52  MH
  - Multi-BoxmenÅ: Nochmal etwas verfeinert

  Revision 1.6  2001/07/08 13:18:21  MH
  - MultiBoxFunktion [RFC/PPP] aus PreRelease uebernommen

  Revision 1.5  2000/04/10 22:13:08  rb
  Code aufgerÑumt

  Revision 1.4  2000/04/09 18:20:25  openxp
