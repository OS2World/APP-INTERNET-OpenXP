...
{ --------------------------------------------------------------- }
{ $Id: xp4e.pas,v 1.44 2001/09/07 09:39:14 MH Exp $ }
...
uses  xpglobal, crt,dos,typeform,fileio,inout,keys,maske,datadef,database,
      winxp,win2,dosx,maus2,resource, xp0,xp1,xp1input,xp3;
...
procedure ReadDirect(txt:atext; var empf,betr,box:string; pmonly:boolean;
                     var brk:boolean);
procedure msgdirect;
procedure msgid_direct;
procedure msgid_edit;
...
function usertest(var s:string):boolean;
var p, alen, dlen, blen, slen, ppos : byte;
    adom : string[60];
begin
  if trim(s)='' then
    ...
    p:=cpos('@',s);
    ppos:=pos('.',mid(s,p+1));
    adom:=ntAutoDomain(pbox,false);
    dlen:=length(adom);
    blen:=length(getfield(pb_field));
    slen:=length(s);
    if ((p=0) and ((slen+blen+dlen+2)>AdrLen)) or
       ((p>0) and (ppos=0) and ((slen+dlen)>eAdrLen)) or
       (slen>AdrLen) then begin
      if p=0 then
        alen:=AdrLen-2-blen-dlen
      else
        if ppos=0 then
          alen:=eAdrLen-dlen
        else
          alen:=AdrLen;
      usertest:=false;
      fehler('Das Feld darf '+strs(alen)+' Zeichen nicht Åberschreiten');
      exit;
    end;
    usertest:=true;
    if p>0 then
...
begin
  ...
  if MarkUnversandt and (left(dbReadStr(mbase,'brett'),1)='U') then begin
    ...
    new(hdp);
    readheader(hdp^,hds,true);
    if (hdp^.pmreplyto<>'') and (length(hdp^.pmreplyto)<eAdrLen) then
      if ReadJN(getres(2739),true) then { 'Vertreter der Nachricht editieren' }
        suchname:=hdp^.pmreplyto else
  ...
  (* siehe Usertest...
  if (length(suchname)=eAdrLen) then begin { Adresse wahrscheinlich zu lang }
    GetMsgBrettUser:=false;
    fehler('Adresse zu lang! User kann nicht aufgenommen werden.');
    exit;
  end;
  *)
...
begin
  ...
  if (box='') and edit then pb_netztyp:=100 else
...
begin
  ...
  if left(dbReadStr(bbase,'brettname'),1)<'A' then begin
    rfehler(2712);  { 'Bei internen Brettern nicht mîglich.' }
    exit;
    end;
...
begin
  ...
  if dbXsize(ubase,'adresse')=0 then adresse:=user
    ...
    if adresse='' then adresse:=user else
    if adresse<>user then
      if ReadJN(getres(2740),true) then begin { 'Vertreter des Users editieren' }
        dbSeek(ubase,uiName,ustr(adresse));
        if not dbFound then
          if ReadJN(getres(2709),true) then   { 'User nicht in der Datenbank - neu anlegen' }
            makeuser
          else
...
begin
  ...
  if (box='') and not ReadJN(getres(2711),true)   { 'Keine Box angegeben - internes Brett anlegen' }
  then exit;
  if box<>'' then begin
    dbOpen(d,BoxenFile,1);
  ...
  if ntBoxNetztyp(box) in [nt_UUCP,nt_PPP] then inc(flags,16);
...
begin
  ...
  if not brk then begin
    if n=2 then
      uucp:=iif(ntBoxNetztyp(s) in [nt_UUCP,nt_PPP],16,0);
...
begin
  ...
  if (not exist(reqfname)) or (_filesize(reqfname)=0) then begin
    fehler(getres(2743)); { 'Keine Anforderungen vorhanden!' }
    exit;
  end else reset(reqfile); ii:=0;
  with midrec do begin
    for i:=1 to filesize(reqfile) do begin
      read(reqfile,midrec);
      if (aktiv<>0) then begin
        zst:=false; del:=false; inc(ii);
        if box='*' then begin
          box:=DefaultBox;
          nos:=true;
        end else nos:=false;
        dialog(58,9,getres2(2718,8),x,y);  { 'Edit Message-Request' }
        maddstring(3,2,getres2(2718,1),box,BoxRealLen,BoxNameLen,'>');
        mappcustomsel(BoxSelproc,false);                  { 'Server    ' }
        mset0proc(dnotepollbox);
        msetvfunc(dtestpollbox);
        brettfld:=-1; userfld:=2; adrfieldpos:=-1;
        ntyp_y:=y+1; pb_field:=1;
        pb_Netztyp:=ntBoxNetztyp(box);
        maddbool(3,4,getres2(2718,9),nos);   { 'Nachricht beim ersten erreichbaren Server holen' }
        delfirst(mid); dellast(mid);
        maddstring(3,6,getres2(2718,5),mid,41,midlen,''); { 'Message-ID' }
        maddbool(3,8,getres2(2718,6),del);       { 'Anforderung lîschen' }
        maddbool(31,8,getres2(2718,7),zst);      { 'ZÑhler zurÅcksetzen' }
        readmask(brk);
        freeres;
        enddialog;
        if brk then break else begin
          {'Diese Funktion wird nur bei RFC/PPP-Boxtypen unterstÅtzt'}
          if pb_Netztyp<>nt_PPP then fehler(getres(170)) else
          { 'UngÅltige Message-ID erkannt' }
          if (pos('@',mid)=0) and not del then fehler(getres(171)) else begin
          { 'Nachrichten beim ersten erreichbaren Server holen?' }
            {if not del then
              if ReadJN(copy(getres(345),11,49),false) then box:='*';}
            if zst then zustand:=0;
            if del and (aktiv in [100..102]) then
              del:=ReadJN(getres(2744),false); { 'Wollen Sie diesen E-Mailrequest wirklich lîschen' }
            if del then begin
              aktiv:=0;
              zustand:=0;
              box:='';
              ...
              mid:=mailstring(mid,false);
              if mid[1]<>'<' then mid:='<'+mid;
              if mid[length(mid)]<>'>' then mid:=mid+'>';
              if nos then box:='*' else
              box:=left(BoxFilename(box),8);
            end;
            seek(reqfile,i-1);
  ...
  close(reqfile);
  if ii=0 then fehler(getres(2743)); { 'Keine aktiven Anforderungen vorhanden!' }
end;
...
var
  ...
  box : string[boxnamelen];
begin
  dialog(58,5,getres2(2718,4),x,y);                 { 'Message-ID Request' }
  mid:=''; box:=defaultbox;
  maddstring(3,2,getres2(2718,1),box,BoxRealLen,BoxNameLen,'>');
  mappcustomsel(BoxSelproc,false);                  { 'Server    ' }
  mset0proc(dnotepollbox);
  msetvfunc(dtestpollbox);
    ...
    ntyp_y:=y+1; pb_field:=1;
    pb_Netztyp:=ntBoxNetztyp(box);
  maddstring(3,4,getres2(2718,5),mid,40,midlen,''); { 'Message-ID' }
  readmask(brk);
  freeres;
  if not brk and (mid<>'') then begin
    {'Diese Funktion wird nur bei RFC/PPP-Boxtypen unterstÅtzt'}
    if pb_Netztyp<>nt_PPP then fehler(getres(170)) else
    { 'UngÅltige Message-ID erkannt' }
    if (pos('@',mid)=0) or (mid<>mailstring(mid,false)) then
      fehler(getres(171)) else
    begin
      { 'Nachrichten beim ersten erreichbaren Server holen?' }
      if ReadJN(copy(getres(345),11,49),false) then box:='*';
      mid:=mailstring(mid,false);
      ...
      for i:=1 to filesize(reqfile) do begin
         read(reqfile,midrec);
         if (midrec.aktiv<>0) and (midrec.mid=mid) then begin
...
var brk  : boolean;
    ...
    b    : byte;
begin
  if (ParMailto='') then begin
    empf:=''; betr:='';
  end else
  {if (pos('BODY',UStr(ParMailto))=0) and (pos('CC',UStr(ParMailto))=0) and
     (pos('BCC',UStr(ParMailto))=0) and (pos('&',ParMailto)=0) then}
  begin
    if pos(';',ParMailto)=0 then b:=pos('?',ParMailto) else
      b:=pos(';',ParMailto);
    if b<>0 then empf:=copy(ParMailto,1,b-1) else
    if (pos('&',ParMailto)=0) and (pos('SUBJECT=',UStr(ParMailto))=0) then empf:=ParMailto else empf:='';
    b:=pos('SUBJECT=',UStr(ParMailto));
    if (b<>0) and (pos('BODY',Ustr(ParMailto))=0) then
    betr:=copy(ParMailto,b+8,length(ParMailto)-b+9) else betr:='';
    ParMailto:='';
  end;
...
begin
  ...
  if (newbox<>'') and not brk then begin
    ...
    uucp:=iif(ntBoxNetztyp(newbox) in [nt_UUCP,nt_PPP],16,0);
    attrtxt(col.coldialog);
          ...
          if ustr(dbReadStr(d,'pollbox'))=oldbox then
            ...
            then begin
              ...
              if i=1 then begin
                ...
                Brett:=dbReadStr(d,'brettname');
                if bretter and (Brett[1]='A') then begin
                  delfirst(Brett);
                  if ntKleinBrett(ntBoxNetztyp(newbox)) then
                    Brett:=LStr(Brett)
                  else
                  if ntGrossBrett(ntBoxNetztyp(newbox)) then
                    Brett:=UStr(Brett);
                  Brett:='A'+Brett;
                  dbWrite(d,'brettname',Brett);
                end;
              end;
            end;
          dbNext(d);
...
{
  $Log: xp4e.pas,v $
  Revision 1.44  2001/09/07 09:39:14  MH
  - Letzten Fix optimiert (doppelter Aufruf verhindert)

  Revision 1.43  2001/09/03 09:32:11  MH
  - Letzten Fix ergÑnzt

  Revision 1.42  2001/09/02 11:42:35  MH
  - Fix: Absturz beim Anlegen von zu langen Absendern

  Revision 1.41  2001/07/22 12:06:29  MH
  - FindFirst-Attribute ge‰ndert: 0 <--> DOS.Archive

  Revision 1.40  2001/06/18 20:17:31  oh
  Teames -> Teams

  Revision 1.39  2001/04/20 16:59:27  MH
  BugFix (/mailto:)
  - wurde versucht per F2 den EmpfÑnger zu Ñndern, stÅrzte XP2 in den Abgrund

  Revision 1.38  2001/01/06 22:43:06  MH
  - Bei editieren von internen Brettern, auf die Anzeige der
    DefaultBox verzichten

  Revision 1.37  2001/01/06 17:11:13  MH
  - Bei box='' internes Brett so 'A/brett' anlegen, nicht '$/brett'

  Revision 1.36  2000/12/31 17:12:41  MH
  Fix Mid-Request:
  - Nach einer Suche oder auch direkt konnten ungÅltige
    MsgIDs verwendet werden

  Revision 1.35  2000/12/09 19:36:58  MH
  MessageID-Req-Editor:
  - Lîschmîglichkeit bei falscher MID

  Revision 1.34  2000/11/22 21:52:29  MH
  Fix: Dialoge inkonsistent

  Revision 1.33  2000/11/20 18:00:11  MH
  Edit Message-Request:
  - Dialog 'Nachricht beim ersten erreichbaren Server holen'

  Revision 1.32  2000/11/20 17:25:15  MH
  Edit Message-Request:
  - Einen Eintrag in den Resourcen hinzugefÅgt
  - Behandlung von spitzen Klammern optimiert

  Revision 1.31  2000/11/19 17:25:44  MH
  Message-ID Request:
  - Stornierung ermîglicht

  Revision 1.30  2000/11/19 12:40:25  MH
  Message-Request-Editor:
  - TopOfMail gesondert behandeln

  Revision 1.29  2000/11/19 11:02:38  MH
  Message-Request-Editor:
  - VollstÑndig in den Resourcen aufgenommen

  Revision 1.28  2000/11/19 10:30:23  MH
  Message-Request-Editor implementiert
  - First version...

  Revision 1.27  2000/11/12 12:24:14  MH
  Userswitch Åberarbeitet

  Revision 1.26  2000/11/09 21:45:12  MH
  Fix: Server > 15 Zeichen

  Revision 1.25  2000/10/10 07:54:09  rb
  Supersedes-Support fertiggestellt
  - Autoversand
  - SV: Einzelnachrichten

  Revision 1.24  2000/10/08 23:36:40  MH
  MsgID-Req. ins Resourcenfile Åbernommen

  Revision 1.23  2000/10/07 14:19:09  MH
  Nachricht/Message-ID Request...
  - Wenn das MID-Feld leer steht, dann kînnen wir auf
    Meldungen verzichten

  Revision 1.22  2000/10/07 13:16:42  MH
  HDO: Im MenÅ Nachricht/Message-ID Request...
  ...kann nun eine solche direkt Åbergeben werden

  Revision 1.21  2000/09/09 15:15:21  MH
  Bei anlegen interner Bretter konnte die falsche Gruppe gesetzt werden

  Revision 1.20  2000/08/19 07:57:45  MH
  MailTo: Kleine énderung um sicher zu gehen

  Revision 1.19  2000/08/12 10:17:58  MH
  Mailto: Bei Body wird vorerst kein Betreff Åbernommen

  Revision 1.18  2000/08/12 00:41:30  MH
  Mailto-Parameter vorbereitend verfeinert

  Revision 1.17  2000/08/11 15:05:51  MH
  Parameter 'mailto:' startet CrossPoint in 'Nachrichten/Direkt'
  und Åbergibt den EmpfÑnger

  Revision 1.16  2000/08/02 12:24:23  MH
  Fix: Zum Fix zum hinzufÅgen der Bretter

  Revision 1.15  2000/07/29 08:48:51  MH
  *** empty log message ***

  Revision 1.14  2000/07/29 08:46:32  MH
  Fix: Spezial/Edit: Wird keine Box angegeben, wird ein internes
  Brett angelegt

  Revision 1.13  2000/07/29 08:33:08  MH
  Fix: Spezial/HinzufÅgen: Wird keine Box angegeben, wurde man
  gefragt, ob ein 'internes' Brett angelegt werde soll, aber ein
  'normales' Brett angelegt

  Revision 1.12  2000/07/21 17:59:34  MH
  Doppelten Aufruf entfernt

  Revision 1.11  2000/07/21 17:55:11  MH
  Serverwechsel erweitert, damit Klein-Gro·schreibung
  dabei beachtet wird

  Revision 1.10  2000/07/10 19:09:12  MH
  Sourceheader ausgetauscht

  Revision 1.9  2000/06/22 17:47:32  tg
  Y2K-Fix in Autoversand-Datei

  Revision 1.8  2000/06/22 03:47:07  rb
  Haltezeit-Bug bei ver32 gefixt

  Revision 1.7  2000/06/07 15:08:32  MH
  RFC/PPP: Einige Anpassungen korrigiert

  Revision 1.6  2000/06/06 08:00:02  MH
  RFC/PPP: Weitere Anpassungen und Fixes

  Revision 1.5  2000/04/10 22:13:10  rb
  Code aufgerÑumt

  Revision 1.4  2000/04/09 18:24:47  openxp
