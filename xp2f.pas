...
{ --------------------------------------------------------------- }
{ $Id: xp2f.pas,v 1.21 2001/08/03 09:26:25 MH Exp $ }
...
uses xpglobal,crt,typeform,inout,keys,winxp,maske,video,maus2,resource,
     xp0,xp1,xp1help,xp1input,xp1o,xp2,fileio;
...
procedure savecolors(fn:string);
var t : text;
    i : integer;
...
begin
  if (fn='') then begin     { Defaultfile ins XP-Verzeichnis }
    fn:=colcfgfile;
    assign(t,fn);
    rewrite(t)
  end else begin            { alle anderen nach colors\ }
    fn:=GetBareFilename(fn)+'.COL';
    mkdir(copy(ColorDir,1,length(ColorDir)-1));
    if (ioresult=0) then;
  ...
  if ioresult<>0 then begin
    rfehler1(107,ustr(fn));       { 'Fehler beim Schreiben von %s' }
    exit
  end;
  ...
  with col do begin
    ...
    wrl('Msgs'); wrh(colmsgs); wrh(colmsgshigh); wrh(colmsgsinv);
                 wrh(colmsgsinfo); wrh(colmsgsuser); wrh(colmsgsinvuser);
                 { Priority hervorgeh. }
                 wrh(colmsgsprio1); wrh(colmsgsprio2);
                 wrh(colmsgsprio4); wrh(colmsgsprio5);
                 wrhl(colmsgsinvprio);
    wrl('MBox'); wrh(colmbox); wrh(colmboxrahmen); wrh(colmboxhigh);
               { eigene Msgs          Cancel/Supersedes    Priority hervorgeh. }
                 wrh(colmboxehigh);   wrh(colmboxchigh);   wrhl(colmboxphigh);
...
begin
  with col do begin
    at(colmsgsinfo);
    moff;
    wrt(31,4,forms(' /Z-NETZ/CROSSPOINT/XP2-TEAM',50));
    at(colmsgs);
    wrt(31,11,'      619 21.04.00 M.KUHLMANN@XPDEV.ZER    Re: XPý');
    wrt(31,12,' >    354 21.04.00 A.KANNEGIESSER@XPDEV.ZE Re: XPý');
    wrt(31,6,' >  H   0 21.04.00 X.CROSS@POINT.ZER       Re: XPý');
    at(colmsgsuser);
    wrt(31,5,'  +   391 21.04.00 P.MANDRELLA@XP.ZER      XPý');
    at(colmsgsinvuser);
    wrt(31,8,' >    140 21.04.00 T.GOHEL@XPDEV.ZER       Re: XPý');
    at(colmsgsprio1);  { Prio. hervorgeh. }
    wrt(31,13,' >    147 21.04.00 M.HUCKENBECK@XPDEV.ZER  Re: XPý');
    at(colmsgsprio2);
    wrt(31,14,' >    245 21.04.00 R.BOECK@XPDEV.ZER       Re: XPý');
    at(colmsgsprio4);
    wrt(31,15,' >    726 21.04.00 T.JAEHNIGEN@XPDEV.ZER   Re: XPý');
    wrt(31,16,' >    726 21.04.00 M.FOSTER@XPDEV.ZER      Re: XPý');
    at(colmsgsprio5);
    wrt(31,17,' >    316 21.04.00 M.MUELLER@XPDEV.ZER     Re: XPý');
    at(colmsgsinvprio);
    wrt(31,18,' >    512 21.04.00 O.HERTEL@XPDEV.ZER      Re: XPý');
    at(colmsgshigh);
    wrt(31,7,'þ     262 21.04.00 C.FAULHAMMER@XPDEV.ZER  Re: XPý');
    wrt(31,9,'þ     532 21.04.00 F.MOLLENHAUER@XPDEV.ZER Re: XPý');
    at(colmsgsinv);
    wrt(31,10,' >  R   0 21.04.00 P.CROSS@POINT.ZER       Re: XPý');
    at(colmsgs);
    mon;
    clwin(31,80,19,screenlines-2);
    end;
end;
...
begin
  with col do begin
    wwin(35,75,10,18,'Meldung',colmboxrahmen,colmbox);
    mwrt(38,12,'normaler Text');
    at(colmboxhigh);
    mwrt(38,13,'hervorgehobener Text');
    at(colmboxehigh);
    mwrt(38,14,'hervorheben bei eigenen Nachrichten');
    at(colmboxchigh);
    mwrt(38,15,'hervorheben bei Cancel/Supersedes');
    at(colmboxphigh);   { Priority hevorgehoben }
    mwrt(38,16,'hervorheben bei Priority');
  end;
end;
...
begin
  with col do begin
    ...
    wrt(31,3,'Betreff    : NEU: CrossPoint v3.30 (XP2)         ');
    wrt(31,4,'Absender   : VERTEILER@XP2.DE                    ');
    wrt(31,5,'Datum      : Mo 02.10.00, 08:45                  ');
    wrt(31,6,'-------------------------------------------------');
    at(collisttext);
...
begin
  with col do begin
    ...
    wrt(57,10,'³ Einfgen       + ³');
    wrt(57,11,'³ Laden        ^KR ³');
    wrt(57,12,'³ Laden UUE    ^KU ³');
    wrt(57,13,'³ Speichern    ^KW ³');
    wrt(57,14,'ÃÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ´');
    wrt(57,15,'³ Umbruch aus   F3 ³');
    wrt(57,16,'³ Umbruch ein   F4 ³');
    wrt(57,17,'ÃÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ´');
    wrt(57,18,'³ Optionen     ^OR ³');
    wrt(57,19,'ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ');
    wshadow(58,77,8,20);
    at(coleditmenuhi);
    wrt(59,9,'A'); wrt(59,10,'E'); wrt(59,11,'L'); wrt(61,12,'d');
    wrt(59,13,'S'); wrt(59,15,'U'); wrt(60,16,'m'); wrt(59,18,'O');
    at(coleditmenuinv);
    wrt(58,8,' Kopieren       * ');
    at(coledithiinv);
    wrt(59,8,'K');
   ...
   2,3,4 : begin     { Untermen 1.-3. Ebene }
       ...
       8 : repeat    { Nachrichten }
             ...
             case m of
               ...
               7 : cset(colmsgsprio1);   { Prio. hervorgeh. }
               8 : cset(colmsgsprio2);
               9 : cset(colmsgsprio4);
              10 : cset(colmsgsprio5);
              11 : cset(colmsgsinvprio);
             end;
           until m=0;
       9 : repeat    { Message-Box }
             ...
             case m of
               ...
               3 : cset(colmboxhigh);  { vervorgehobener Text }
               4 : cset(colmboxehigh); { eigene Nachrichten   }
               5 : cset(colmboxchigh); { cancel / supersedes  }
               6 : cset(colmboxphigh); { Prio. hervorgeh.     }
             end;
           until m=0;
      ...
      20 : begin { Laden }
             fn:=colordir+'*.COL';
             if not (ReadFilename2(getres(247), fn, false)) then fn:=colcfgfile;
             readcolors(fn);
           end;
      21 : begin { Sichern }
               fn:=colordir+'*.COL';
             if not (ReadFilename2(getres(245), fn, false)) then fn:=colcfgfile;
             savecolors(GetBareFilename(fn)+'.COL');
           end;
      22 : n:=0; { OK }
    end;
  until n=0;
  if (n=0) then if ReadJN(getres(246),true) then   { 'Diese Farben als Standard definieren' }
    savecolors('');
  freeres;
  pophp;
  setcolors;
...
{
  $Log: xp2f.pas,v $
  Revision 1.21  2001/08/03 09:26:25  MH
  - Fix (Load/SaveColor - ColorDir):
    - Verzeichnis wurde nicht erstellt
    - Variable fn zu kurz

  Revision 1.20  2001/07/16 10:02:38  MH
  - Farbsetup erweitert:
    Beim 'Puffereinlesen' werden eigene Nachrichten, Cancel/Supersedes usw.
    in verschiedenen Farben im Betreff angeszeigt

  Revision 1.19  2001/06/19 21:15:27  mm
  Zusatzmenue auf 20 erweitert (von Jochen Gehring)

  Revision 1.18  2001/06/18 20:17:26  oh
  Teames -> Teams

  Revision 1.17  2001/03/22 20:35:33  oh
  -Farbprofile update

  Revision 1.16  2001/03/22 09:28:37  oh
  - Farbprofile werden erst in colors\ gesucht, dann im XP-Verzeichnis

  Revision 1.14  2000/11/19 17:55:01  MH
  -.-

  Revision 1.13  2000/10/26 14:48:54  MH
  -.-

  Revision 1.12  2000/10/25 18:42:07  tj
  Sourceheader aktualisiert

  Revision 1.11  2000/07/12 08:11:51  MH
  Weiteren Entwickler in die Farbconfig aufgenommen

  Revision 1.10  2000/05/31 21:12:40  rb
  Dateinamen auf dem CVS wieder kleingemacht

  Revision 1.9  2000/05/05 20:14:56  MH
  Fix: Priority-Farbhervorhebung

  Revision 1.8  2000/05/01 11:36:57  MH
  FarbSetup: Optionsmenn Editor aktuallisiert

  Revision 1.5  2000/04/10 22:13:09  rb
  Code aufger„umt

  Revision 1.4  2000/04/09 18:22:45  openxp
