...
{ --------------------------------------------------------------- }
{ $Id: xp6.pas,v 1.57 2002/02/20 19:56:02 rb Exp $ }
...
uses xpglobal,crt,dos,typeform,fileio,inout,keys,datadef,database,maske,
     crc16,lister,winxp,montage,stack,maus2,resource,xp0,xp1,xp1input,xp2c,
     xp_des,xpe;
...
var i : integer;
begin
  if (edis=1) or not (netztyp in [nt_UUCP,nt_PPP]) or not NewsgroupDisp then
...
begin
  ...
  if edpush then begin
    ...
    if verteiler then write(forms(getres2(611,40)+vert_name(empfaenger),79))
      ...
      else write(forms(getres2(611,41)+left(uucpbrett(empfaenger,edis),55)+
...
begin
  ...
  procedure changeempf;  { EmpfÑnger der Mail Ñndern }
  var kbs : boolean;
  begin
    kbs:=kb_shift;
    pm:=cpos('@',empfaenger)>0;
    if pm then adresse:=empfaenger else begin
      adresse:=uucpbrett(empfaenger,2);
      fidoto:=BrettAlle;
    end;
    sData^.PmReplyTo:='';
    sData^.AmReplyTo:='';
...
procedure NachrichteninhaltErzeugen;
begin
...
fromstart:
  ...
  if pm then begin
    ...
    if newuseribm then umlaute:=0 { NewUserIBM berÅcksichtigen }
     else umlaute:=1;             { IBM=0, ASCII=1, ISO=2      }
      empfneu:=true;
      verteiler:=false;
    ...
    if not dbFound then begin
      ...
      dbReadN(bbase,bb_adresse,_AmReplyTo); { Brett-Vertreter }
      flags:=dbReadInt(bbase,'flags');
      if flags and 8<>0 then { Schreibsperre }
        if (_AmReplyTo='') or ((_AmReplyTo<>'') and (flags and 32<>0)) then begin
        _AmReplyTo:='';
        rfehler(450); { 'Schreibzugriff auf dieses Brett ist gesperrt' }
        goto xexit;
      end;
      if ((_AmReplyTo<>'') and (flags and 32=0)) then begin  { FollowUp-To? }
        pm:=cpos('@',_AmReplyTo)<>0;
        if not pm then dbSeek(bbase,biBrett,'A'+ustr(_AmReplyTo)) else begin
          empfaenger:=_AmReplyTo;
          goto fromstart;
        end;
        if dbFound then
          if ReadJN(getres(623),false) then { 'An Brettvertreter schreiben' }
            dbReadN(bbase,bb_brettname,empfaenger) else begin
              dbSeek(bbase,biBrett,ustr(empfaenger));
              if sendfilename<>'' then sData^.AmReplyTo:=_AmReplyTo;
            end;
      end;
  ...
  FidoBin:=binary and pm and
           ((netztyp=nt_Fido) or
            ((netztyp in [nt_UUCP,nt_PPP]) and (left(empfaenger,length(uuserver))=uuserver)));
  ...
  if not ((registriert.non_uucp and (netztyp<>nt_UUCP)) or
          ...
          binary or TestXPointID or (registriert.ppp and (netztyp=nt_PPP)))
...
{ else
  ...
  if not fileattach then
    ukstring(betreff);
  { 'private Nachricht' / 'interne Nachricht' / 'îffentliche Nachricht' }
  typ:=getres2(611,iif(pm and not brettverteiler,1,iif(grnr=IntGruppe,2,3)));
    ...
    ReadAgain:
      ...
      if spezial then begin
        ...
        case netztyp of
          ...
          nt_PPP,
          nt_UUCP : if n=6 then n:=12        { RFC-Prio    }
                      else if n=7 then n:=10 { Zusatz      }
                      else if n=8 then n:=11 { PGP-Sig     }
                      else if n=9 then n:=0; { ZurÅck      }
        ...
        if n>0 then inc(n,10)
          ...
          p:=pos(upcase(t[1]),getres2(611,25));   { JNI˘MBOCT }
          if p>0 then n:=p;
          end;
        end
      else begin
        repeat
          ...
          n:=readbutton(x+3,y+11,1,getres2(611,28)+
               iifs(binary or (sendflags and sendWAB<>0),'',getres2(611,29)),
                        abs(n),true,t);
        { ' ^Ja ,^Nein,^Intern,^Spezial,˘3E^mp.,^Betreff,B^ox,^Code' ',^Text' }
        until (n>=0) or ((t<>mausmoved) and (t<>mauslmoved));
        if n=4 then begin
          spezial:=true;
          attrtxt(col.coldialog);
        ...
        if n<0 then begin
          ...
          case p of
            ...
            9    : if netztyp in [nt_ZConnect,nt_UUCP,nt_PPP] then n:=20;
            10   : if netztyp in [nt_ZConnect,nt_Fido,nt_Maus,nt_UUCP,nt_PPP] then
                     n:=21;  { PGP }
      ...
      case n of
       ...
       21   : SendPgpOptions;
       22   : begin
               {if not netztyp in [nt_uucp, nt_ppp] then rfehler(622);}
               getprio;
               showflags;
              end
                ...
                if ustr(t)=kopkey then begin
                 ...
                 { if _AmReplyTo<>'' then sData^.AmReplyTo:=_AmReplyTo; }
  ...
  if not verteiler then begin
    ...
    if empfneu then
      if pm then begin
        ...
        {if netztyp=nt_Fido then inc(b,8);} { 8=ASCII-Umlaute }
        ...
        b:=iif((netztyp=nt_UUCP) or (netztyp=nt_PPP),16,0);
    ...
    NachrichteninhaltErzeugen;

    { --- 2. Schritt: Nachricht in mbase/MPUFFER ablegen --------------- }

    bin_msg:=binary and (maxbinsave>0) and (fs>maxbinsave*1024) {and (netztyp<>nt_PPP)};
    if not bin_msg then
      assign(f^,fn^)
    else begin
    ...
    case ntDomainType(netztyp) of    { s. auch XP4O.CancelMassage! }
      ...
      8 : begin
            hdp^.absender:=username+'@'+pointname+domain;
            hdp^.real_box:=box;
          end;
    end;
...
{    if (netztyp<>nt_Fido) or pm {or not XP_ID_AMs}{ then }
      ...
      hdp^.programm:=xp_xp+' '+xp2str+verstr+betastr+pformstr+
    ...
    else if (netztyp in [nt_UUCP,nt_PPP]) and not adrpmonly then
      hdp^.homepage:=wwwHomepage^;
    hdp^.priority:=rfcprio;
    hdp^.xnoarchive:=noarchive;
    ...
    if ntPmReply(netztyp) then
      if _pmReply {or sData^.pm_reply} then inc(hdp^.attrib,AttrPmReply);
    if ControlMsg then inc(hdp^.attrib,AttrControl);
    if (binary and (netztyp in [nt_UUCP,nt_PPP]) and multipartbin) or
...
{    hdsize:=filepos(f2^); }
    ...
    repeat                                   { einzelne Crosspostings in }
      ...
      if NetzTyp = nt_ZConnect then                  { Zconnect-PrioritÑt: }
        if MsgPrio = 10 then Flags := Flags or 16    { Direkt = Hoch }
      else if MsgPrio = 20 then Flags := Flags or 8; { Eilmail = Hîchste }

      case RFCPrio of                                { RFC - PrioritÑt }
        1 : Flags := Flags or 8;                     { Hîchste }
        2 : Flags := Flags or 16;                    { Hoch }
        4 : Flags := Flags or 24;                    { Niedrig }
        5 : Flags := Flags or 32;                    { Niedrigste }
      end;

      flags:=flags or 256; { MyMail }
      ...
      if not pm and (msgCPpos=0) then begin    { Brettdatum neu setzen }
        dbSeek(bbase,biBrett,ustr(empfaenger));
        if not dbFound then
          tfehler('neue Msg: Brett weg??',errortimeout)
        else
  ...
  {if (readmode=1) and (aktdispmode=10) then
    aufbau:=false
  else}
...
var empf,repto : string[AdrLen];
  ...
  begin
    ...
    assign(f,fn);
    GetFAttr(f, Attr);
    if (Attr and ReadOnly)<>0 then SetFAttr(f,0);
    reset(f);
...
{
  $Log: xp6.pas,v $
  Revision 1.57  2002/02/20 19:56:02  rb
  ...
  Revision 1.56  2002/01/01 12:13:18  mm
  - Automatisches Halten selbstgeschriebener PMs unter
    Config/Optionen/Nachrichten/"Eigene PMs halten" konfigurierbar
    (Jochen Gehring)

  Revision 1.55  2001/07/19 14:14:28  MH
  - Ungelesen-Fixes wieder mi·lungen - Neuaufbau bei
    neuen KopienempfÑngern notwendig!

  Revision 1.54  2001/07/15 22:29:18  MH
  - DoSend merkt sich die Position der bbase und setzt sie zurÅck
    (Fix fÅr Mailinglisten usszuschigge - Zugriff auf bbase 'kommentar')

  Revision 1.52  2001/06/26 15:01:56  MH
  - NewsGroupAnzeige bei Replys dem Netztyp anpassen

  Revision 1.51  2001/06/20 09:28:50  MH
  - AmReplyTo auch bei Text/Binary-Versandt

  Revision 1.50  2001/06/18 20:17:33  oh
  Teames -> Teams

  Revision 1.49  2001/04/04 19:57:02  oh
  -Timeouts konfigurierbar

  Revision 1.48  2000/11/18 11:15:01  MH
  Brettvertreter und Schreibsperre bei FollowupTo beachten
  - nun auch bei Mailadressen

  Revision 1.47  2000/11/17 21:05:34  MH
  Brettvertreter und Schreibsperre bei FollowupTo beachten
  - In die Resourcen aufgenommen
  - Mailadressen ignorieren

  Revision 1.46  2000/11/17 19:44:43  rb
  Codeteile ausgelagert

  Revision 1.45  2000/11/17 18:55:18  MH
  Brettvertreter und Schreibsperre bei FollowupTo beachten

  Revision 1.44  2000/11/01 15:34:42  MH
  -.-

  Revision 1.43  2000/10/28 07:55:16  MH
  Mailerzeile incl. Support-Url

  Revision 1.42  2000/10/26 17:13:32  MH
  énderung der Mailer- und Tearlinezeile nun wirksam

  Revision 1.41  2000/10/26 14:49:21  MH
  Ein reiner Brettverteiler erfÑhrt eine Sonderbehandlung

  Revision 1.40  2000/10/13 20:07:00  MH
  Followup-To for PMs only...

  Revision 1.39  2000/10/13 14:00:18  MH
  Replace:
  - Wir verabschieden uns von X-XP-MSG und setzen nun die Datenbank ein

  Revision 1.38  2000/10/12 10:04:07  MH
  RFC/PPP: E-Mailadresse immer aus 'username@pointname.domain' bilden

  Revision 1.37  2000/10/12 02:32:00  MH
  Fix: öbernahme Replyto bei neuer Boxauswahl

  Revision 1.36  2000/10/12 01:50:59  MH
  Baustelle aufgerÑumt

  Revision 1.35  2000/10/10 15:28:31  MH
  ReplaceOwn nun Client- und Netz unabhÑngig...

  Revision 1.34  2000/10/10 07:54:10  rb
  Supersedes-Support fertiggestellt
  - Autoversand
  - SV: Einzelnachrichten

  Revision 1.33  2000/09/28 22:17:58  rb
  Patch von Frank Ellert fÅr fido_to komplettiert

  Revision 1.32  2000/09/21 21:59:30  rb
  Codeteile ausgelagert

  Revision 1.31  2000/09/20 21:42:07  oh
  Patch 1 (Frank Ellert)

  Revision 1.30  2000/08/27 16:03:21  MH
  JG: Verteiler im Sendefenster erlaubt bei:
  'Kopien an' und 'EmpfÑnger Ñndern'

  Revision 1.29  2000/08/05 18:43:04  MH
  Fix: MAILER-DAEMON: BoxPar werden nun zuvor gesichert

  Revision 1.28  2000/08/05 09:45:09  MH
  Unnîtige Fehlermeldung entfernt

  Revision 1.27  2000/08/02 16:03:04  MH
  *** empty log message ***

  Revision 1.26  2000/07/31 15:11:31  MH
  MAILER-DAEMON kann nun fÅr jede Box separat konfiguriert werden

  Revision 1.25  2000/07/21 15:27:51  MH
  Leerstellen zwischen '@' beim antworten von Nachrichten entfernt

  Revision 1.24  2000/07/18 12:43:16  MH
  Fix: Sendefenster 'EmpfÑnger Ñndern' Brettverterter
       wird nun wie definiert Åbernommen

  Revision 1.23  2000/07/12 18:26:42  MH
  Baustelle aufgerÑumt

  Revision 1.22  2000/07/12 17:12:42  MH
  Sendefenster: EmpfÑnger Ñndern beachtet nun auch Schreibsperre
                und Brettvertreter

  Revision 1.20  2000/07/11 19:19:37  MH
  Fix: EmpfÑnger Ñndern: Screen wurde zerstîrt

  Revision 1.19  2000/07/07 15:14:19  MH
  Sendefenster (EmpfÑnger Ñndern):
  - EmpfÑnger kann nun wahlweise immer Brett oder User sein

  Revision 1.18  2000/07/06 16:35:55  MH
  - Mailerstring angepasst

  Revision 1.17  2000/07/05 23:11:56  MH
  Mailerzeile:
  - mit (XP2) gekennzeichnet: neue Variable (teamstr) beachten

  Revision 1.16  2000/06/24 16:47:10  tg
  Beta-String wird nun korrekt uebernommen

  Revision 1.15  2000/06/22 03:47:08  rb
  Haltezeit-Bug bei ver32 gefixt

  Revision 1.14  2000/06/07 15:07:50  MH
  RFC/PPP: Einige Anpassungen korrigiert

  Revision 1.13  2000/06/06 15:43:49  MH
  Code aufgeraeumt

  Revision 1.12  2000/06/06 14:59:13  MH
  ReadOnly-Binaries koennen wieder versendet werden

  Revision 1.11  2000/05/22 12:35:30  MH
  MAILER-DAEMON: Fuer autom. Empfangsbestaetigung jetzt konfigurierbar (C/O/E)

  Revision 1.10  2000/05/21 14:38:34  MH
  FÅr RFC/PPP gilt fÅr EmpfangsbeststÑtigungen: username@absender (Floyds Mailer) statt MAILER-DAEMON@absender (Floyds Mailer)

  Revision 1.9  2000/05/17 17:27:11  MH
  Neuer Boxentyp: RFC/PPP

  Revision 1.8  2000/04/30 13:41:18  MH
  Empfaengertaste im Sendefenster

  Revision 1.6  2000/04/10 22:13:11  rb
  Code aufgerÑumt
