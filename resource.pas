...
{ --------------------------------------------------------------- }
{ $Id: resource.pas,v 1.7 2001/07/24 16:27:35 mm Exp $ }
...
var   block  : packed array[1..maxblocks] of rblock;
      ...
      clsize : word;         { Cluster-Grî·e }
      clindex: ^tindex;      { Cluster-Index  }
      clcont : barrp;        { Cluster-Inhalt }
      clcsize: word;         { Grî·e des Inhalts }
      clbnr  : integer;
...
begin
  ...
  fm:=filemode; 
  {$IFDEF VirtualPascal }
  filemode:=Open_Access_ReadOnly or Open_Share_DenyWrite;
  {$ELSE }
...
{$IFDEF BP }
        if emspages>0 then EmsEinblenden(bnr);
{$ENDIF }
        if (inr<>clnr) or (bnr<>clbnr) then begin
...
{
  $Log: resource.pas,v $
  Revision 1.7  2001/07/24 16:27:35  mm
  - filemode in OpenResource fuer 32Bit-Version korrigiert

  Revision 1.6  2000/10/07 00:03:58  rb
  MO: Bugfix fÅr Resourcencaching

  Revision 1.5  2000/06/08 20:05:15  MH
  Teamname geandert

  Revision 1.4  2000/04/09 18:14:54  openxp
