...
{ --------------------------------------------------------------- }
{ $Id: xp1.pas,v 1.19 2001/10/29 19:28:22 MH Exp $ }
...
function geti_(var su:string; v:string; var i:integer16):boolean;
...
procedure _wipe(fn:pathstr);
procedure ExWipe(fn:pathstr);
...
begin
  ...
  if screenlines < 25 then screenlines:=GetScreenlines;
...
{ screenlines:=25;
  setscreensize(false); }
{$IFDEF OS2 }
  writeln;      { TJ 260500 - wegem dem Infobalken in den VIO Screen's }
{$ENDIF }
  if deutsch then
    case joke of
...
begin
  if auto then
    tfehler(txt,errortimeout_short)
  else
    fehler(txt);
end;
...
begin
  if auto then
    trfehler(nr,errortimeout_short)
  else
    rfehler(nr);
end;
...
var x,y   : byte;
  ...
  begin
    inc(fpos,n);
    if box then begin
      if fsize>=(401*1024*1024) then n:=((fpos div fsize) * 50) else
      if fsize>=(400*1024*1024) then n:=((fpos * 2 div fsize) * 25) else
      if fsize>=(200*1024*1024) then n:=((fpos * 5 div fsize) * 10) else
      if fsize>=(80*1024*1024)  then n:=((fpos * 10 div fsize) * 5) else
      if fsize>=(40*1024*1024)  then n:=((fpos * 25 div fsize) * 2) else
        n:=(fpos * 50 div fsize);
      mwrt(x+3,y+2,dup(system.round(n),'≤'));
   end;
  end;
...
begin
  ...
  erase(f);
  if (ioresult<>0) then
    trfehler1(4,ustr(fn),errortimeout);   { 'Kann '+ustr(fn)+' nicht lîschen!?' }
end;
...
begin
  ...
  if (ioresult=0) then begin
    l:=filesize(f);
    bufs:=min(min(maxavail,8192),l);
    getmem(buf,bufs);
    fillchar(buf^,bufs,85); { 01010101 }
    while(l>0) do begin
      blockwrite(f,buf^,min(bufs,l),rc);
      dec(l,rc);
    end;
    freemem(buf,bufs);
    close(f);
...
begin
  ...
  if p<>'' then begin
    ...
    chdir(p);
    if (ioresult<>0) then
      trfehler1(5,ustr(p),errortimeout);   { ungÅltiges Verzeichnis: }
    end;
end;
...
begin
  if memavail<=size+16 then begin
    if wait then trfehler(6,errortimeout)  { 'zu wenig freier Speicher' }
    else rfehler(6);
    testmem:=false;
    end
  else
...
{
  $Log: xp1.pas,v $
  Revision 1.19  2001/10/29 19:28:22  MH
  - [JG/ASM] Fix (ListDisplay): Ohne Spaltenumbruch in Spalte 80
    konnte es bei Hervorhebungen zum auseinanderrei·en des Satzes
    kommen.

  Revision 1.18  2001/07/22 13:15:37  MH
  - sound <--> PlaySound fuer VP

  Revision 1.17  2001/06/19 21:15:26  mm
  Zusatzmenue auf 20 erweitert (von Jochen Gehring)

  Revision 1.16  2001/04/04 19:57:00  oh
  -Timeouts konfigurierbar

  Revision 1.15  2001/01/01 23:39:12  MH
  Fix:
  - Fortschrittsanzeige bei gro·en Dateien

  Revision 1.14  2000/11/27 18:04:14  rb
  Zeilenzahl wird jetzt richtig gesetzt, wenn ein Font geladen wird

  Revision 1.13  2000/10/25 09:50:21  rb
  _wipe beschleunigt

  Revision 1.12  2000/10/13 21:12:21  oh
  -PGP Update

  Revision 1.10  2000/06/22 03:47:05  rb
  Haltezeit-Bug bei ver32 gefixt

  Revision 1.9  2000/06/08 20:06:11  MH
  Teamname geandert

  Revision 1.8  2000/05/26 11:25:11  tj
  kleiner Fehler gefixt

  Revision 1.7  2000/05/25 23:20:18  tj
  Bugfixes im Bildschirmaufbau

  Revision 1.6  2000/04/11 20:28:41  oh
