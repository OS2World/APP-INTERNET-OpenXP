...
const max_arc = 3;   { maximale verschachtelte Archivdateien }
      suchlen = 73;  { maximale LÑnge des Suchstrings        }
...
{$ENDIF}
  ...
  { Leerzeichen Links und rechts lîschen, Tilden links ebenfalls }
...
{--Start der Suche--}
    ...
    if (suchfeld='MsgID') and NOT MID_teilstring then begin      {-- Suche: Message-ID  --}
      ...
      if not brk then begin
        ...
        begin          { mAx: spÑter einmal begutachten, deshalb deaktiviert }
...
begin
  ...
  begin
    ...
    if (box='') or (nr=1) or (brett[1]='1') or (brett[1]='$') then begin
      fehler(getres(490)); { 'Bei internen Brettern nicht mîglich' }
      exit;
    end;
    boxfn:=ustr(boxfilename(box));
    if (ntBoxNetztyp(box)<>nt_PPP) then begin
      fehler(getres(489)); { 'Nur beim RFC/PPP-Boxtyp mîglich' }
      exit;
    end;
    fsize:=_filesize(ownpath+boxfn+'.RC');
    if not exist(ownpath+boxfn+'.RC') or (fsize=0) then begin
      fehler(getreps(491,ustr(box))); { 'FÅr %s bestehen noch keine Bestellungen' }
      exit;
    end;
    ...
    if i=1 then begin
      if rhdo then
        rmessage(486) else { 'HeaderOnly zurÅckgesetzt' }
      rmessage(487);       { 'HeaderOnly gesetzt'       }
      wkey(1,false);
      closebox;
    end else hinweis(getres(488)); { 'Bitte Åber Brettmanager bestellen' }
  end;
...
end.
{
  ...
  HDO-Rev.:
  - Moment-Anzeige bei RC-Dateien grî·er 250kB
  ...
  WPOP Error endgÅltig beseitigt
  ...
  MidSuche: Abfrage fÅr Fehlermeldung erweitert
  ...
  HDO-Request in Spezial-Suche hinzugefÅgt

  Revision 1.15  2000/09/03 09:56:17  MH
  SuchOption 'aiÑ or ai' ist vom 1. Eintrag in RQ abhÑngig,
  deshalb nun ein UpperString zum Vergleich verwenden, da
  dieser von wem auch immer geÑndert wurde und die Funktion
  sonst au·er Kraft gesetzt wird.

  Revision 1.14  2000/09/03 09:42:44  MH
  Headeronly in der Spezialsuche hinzugefÅgt
