...
{ --------------------------------------------------------------- }
{ $Id: xpauto.pas,v 1.9 2001/07/22 12:06:58 MH Exp $ }
...
begin
  ...
  with ar do begin
    ...
    if (datei<>'') and not exist(datei) then
      trfehler1(2201,fitpath(datei,42),errortimeout)    { 'AutoVersand - Datei "%s" fehlt!' }
    else if (datei<>'') and (_filesize(datei)=0) then
      trfehler(2202,errortimeout)   { 'AutoVersand - 0-Byte-Datei wurde nicht verschickt' }
    else begin
      tmp:=(datei='');
      if tmp then begin
        datei:=TempS(1000);
...
var sr    : searchrec;
  ...
  begin
    if first then findfirst(AutoxDir+'*.'+ext,DOS.Archive,sr)
    else findnext(sr);
    first:=(doserror<>0);
    find:=not first;
  end;
  ...
  begin
    ...
    if not exist('MAGGI.EXE') then
      ...
      box:=NamePollbox;
      if box='' then
        trfehler1(2204,sr.name,errortimeout)   { 'Kann %s nicht einlesen - ungÅltige Pollbox' }
      else begin
        ReadBoxpar(0,box);
        shell('MAGGI.EXE -sz -b'+box+' -h'+boxpar^.MagicBrett+' '+
              AutoxDir+sr.name+' MPUFFER',300,3);
        if errorlevel<>0 then
          trfehler1(2205,sr.name,errortimeout)   { '%s: Fehler bei Nachrichtenkonvertierung' }
        else begin
  ...
  begin
    if not exist('ZFIDO.EXE') then
      trfehler(101,errortimeout)     { 'Netcallkonvertierer ZFIDO.EXE fehlt!' }
    else if not IsBox(DefFidoBox) then
      trfehler(2207,errortimeout)     { 'Keine gÅltige Fido-Stammbox gewÑhlt' }
    else begin
      ReadBoxpar(0,DefFidoBox);
      shell('ZFIDO.EXE -fz -h'+BoxPar^.MagicBrett+' '+AutoxDir+'*.PKT '+
            'FPUFFER -w:'+strs(screenlines),300,3);
      if errorlevel<>0 then
        trfehler(2208,errortimeout)   { 'Fehler bei Fido-Paketkonvertierung' }
      else begin
        { 27.01.2000 robo - Serverbox bei Fido aus Pfad nehmen }
{
        ...
        { /robo }
          findfirst(AutoxDir+'*.PKT',DOS.Archive,sr);
  ...
  var t1,t2 : text;
    ...
    begin
      tfehler(getres2(2200,1)+getreps2(2200,nr,txt),errortimeout);   { 'AutoExec-Fehler: ' }
      freeres;
    end;
  ...
  begin
    ...
    if not IsBox(box) then
      trfehler1(2209,box,errortimeout)    { 'IPS - ungÅltige Box: %s' }
    else
...
end.
{
  ...
  Revision 1.4  2000/04/09 18:29:02  openxp
