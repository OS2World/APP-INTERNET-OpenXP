...
{ --------------------------------------------------------------- }
{ $Id: xp4o3.pas,v 1.9 2001/08/13 00:14:55 MH Exp $ }
...
uses xpglobal,dos,typeform,fileio,datadef,database,inout,keys,resource,
     xp0,xp1,xp1input;
...
begin
  ...
  with fkeys[nr]^[nn] do begin
    ...
    setfn;
    extract_msg(ntyp,iifs(ntyp=3,grQuoteMsk{QuoteMsk},''),fn,false,1);
    end;
  __getfilename:=fn;
end;
...
begin
  ...
  ReadHeader(hdp^,hds,false);
  { 03.02.2000 robo }
  if (cpos('@',hdp^.PmReplyTo)>1) and (length(hdp^.PmReplyTo)>2) and not askreplyto then
  { /robo }
    abs:=hdp^.PmReplyTo
  else begin
    ...
    then begin
      ...
      anz:=0;
      { 03.02.2000 robo }
      if (cpos('@',hdp^.PmReplyTo)>1) and (length(hdp^.PmReplyTo)>2) then
...
{
  $Log: xp4o3.pas,v $
  Revision 1.9  2001/08/13 00:14:55  MH
  - Letzten Fix noch etwas verschÑrft

  Revision 1.8  2001/08/12 16:23:38  MH
  - Fix bei ReplyTo='-' : soll abstuerzen, konnte ich nicht reproduzieren

  Revision 1.7  2001/07/06 22:32:56  MH
  - optik

  Revision 1.6  2000/09/24 00:42:06  MH
  Requester nun auf R-Taste (deutsch) und G-Taste (englisch)

  Revision 1.5  2000/04/10 22:13:11  rb
  Code aufgerÑumt

  Revision 1.4  2000/04/09 18:25:40  openxp
