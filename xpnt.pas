...
{ --------------------------------------------------------------- }
{ $Id: xpnt.pas,v 1.20 2002/01/24 21:38:17 mm Exp $ }
...
uses   xpglobal, xp0,typeform,datadef,database,xpcrc32;
...
function ntSupersedes(nt:byte):boolean;       { Supersedes-Messages mîglich }
...
begin
  case nt of
    ...
    40       : ntDomainType:=6;   { @point.domain oder @box.domain }
    41       : ntDomainType:=8;   { username@pointname.domain }
  else
    ntDomainType:=0;
  end;
end;
...
function ntKleinBrett(nt:byte):boolean;       { Bretter-Kleinschreibung }
begin
  ntKleinBrett:=(nt in [nt_UUCP, nt_PPP]);
end;
...
begin
  case nt of
    ...
    nt_PPP      : ntName:='RFC/PPP';   { Erste Zeile zum neuen BoxTyp ...;-)}
  else
    ntName:='???';
  end;
end;
...
begin
  case nt of
    nt_Fido,nt_QWK,nt_PPP : ntRelogin:=0;
    nt_GS,nt_Turbo,
    nt_UUCP        : ntRelogin:=1;
    else             ntRelogin:=2;
  end;
...
begin
  ntKomkette:=
    (nt in [nt_Maus,nt_Fido,nt_ZConnect,nt_UUCP,nt_PPP,nt_QWK,nt_Pronet,nt_Turbo])
...
begin
  ntRfcCompatibleID:=(nt=nt_ZConnect) or (nt=nt_Magic)
                      or (nt=nt_UUCP) or (nt=nt_PPP);
end;
...
begin
  ntAdrCompatible:=(n1=n2) or
                   (((n1=nt_ZConnect) or (n1=nt_UUCP) or (n1=nt_PPP)) and
                     (n2 in [nt_ZConnect,nt_UUCP,nt_PPP,nt_Maus]));
end;

function ntEmpfBest(nt:byte):boolean;
begin
  ntEmpfBest:=(nt=nt_Fido) or (nt=nt_ZConnect) or (nt=nt_Magic) or
              (nt=nt_UUCP) or (nt=nt_PPP);
end;


function ntMsg0(nt:byte):boolean;             { Nachricht darf leer sein }
begin
  ntMsg0:=(nt=nt_ZConnect) or (nt=nt_Fido) or (nt=nt_UUCP) or (nt=nt_PPP);
end;
...
function ntRealname(nt:byte):boolean;         { Realnames mîglich }
begin
  ntRealname:=nt in [nt_ZConnect,nt_Magic,nt_Pronet,nt_UUCP,nt_PPP,nt_Turbo];
end;


function ntRealUmlaut(nt:byte):boolean;       { Umlaute im Realname }
begin
  ntRealUmlaut:=nt in [nt_Magic,nt_Pronet,nt_UUCP,nt_PPP];
end;


function ntHeaderUmlaut(nt:byte):boolean;     { Umlaute in Keywords etc. }
begin
  ntHeaderUmlaut:=nt in [nt_ZCONNECT,nt_Magic,nt_Pronet,nt_UUCP,nt_PPP];
end;


function ntCancel(nt:byte):boolean;           { Cancel-Messages mîglich }
begin
  ntCancel:=(nt=nt_UUCP) or (nt=nt_Maus) or (nt=nt_PPP);
end;

function ntSupersedes(nt:byte):boolean;       { Supersedes-Messages mîglich }
begin
  ntSupersedes:=(nt=nt_UUCP) or (nt=nt_PPP);
end;
...
function ntPmReply(nt:byte):boolean;          { attrPmReply erzeugen }
begin
  ntPmReply:=(nt=nt_Maus) or (nt=nt_UUCP) or (nt=nt_PPP);
end;


function ntFollowup(nt:byte):boolean;         { Followup-To mîglich }
begin
  ntFollowup:=(nt=nt_ZConnect) or (nt=nt_UUCP) or (nt=nt_PPP);
end;


function ntCrossAM(nt:byte):boolean;          { AM-Crosspostings mîglich }
begin
  ntCrossAM:=(nt=nt_UUCP) or (nt=nt_PPP) or ((nt=nt_ZConnect) and zc_xposts);
end;

function ntCrossPM(nt:byte):boolean;          { PM-Crosspostings mîglich }
begin
  ntCrossPM:=(nt=nt_ZConnect) or (nt=nt_UUCP) or (nt=nt_PPP);
end;


function ntExtProt(nt:byte):boolean;          { externes ö.-Protokoll }
begin
  ntExtProt:=not (nt in [nt_Fido,nt_UUCP,nt_PPP,nt_Turbo,nt_QWK]);
end;
...
function ntOrigWeiter(nt:byte):boolean;       { Weiterleiten mit WAB  }
begin
  ntOrigWeiter:=(nt=nt_ZConnect) or (nt=nt_UUCP) or (nt=nt_PPP) or (nt=nt_Maus);
end;
...
begin
  ntPMTeleData:=(nt=nt_ZConnect) or (nt=nt_UUCP) or (nt=nt_PPP);
end;
...
function ntSec(nt:byte):boolean;              { sekundengenaue Uhrzeit }
begin
  ntSec:=(nt in [nt_ZCONNECT,nt_UUCP,nt_PPP,nt_Magic,nt_Pronet]);
end;
...
function ntForceMailer(nt:byte):boolean;      { '... (unregistriert)' anzeigen }
begin
  ntForceMailer:=(nt in [nt_ZConnect,nt_UUCP,nt_PPP]);
end;
...
begin
  ntPGP:=(nt=nt_ZCONNECT) or
         ((nt=nt_PPP) and PGP_RFC{PPP}) or
         ((nt=nt_UUCP) and PGP_RFC{UUCP}) or
...
function ntBCC(nt:byte):boolean;              { BCC-Option vorhanden }
begin
  ntBCC := (nt in [nt_ZConnect,nt_UUCP,nt_PPP]);
end;


function ntFilename(nt:byte):boolean;         { Dateiname im Header }
begin
  ntFilename := (nt in [nt_ZConnect,nt_UUCP,nt_PPP]);
end;
...
{
  $Log: xpnt.pas,v $
  Revision 1.20  2002/01/24 21:38:17  mm
  ntBetreffLen gibt bei nt_Fido Fido-konforme 70 zurueck

  Revision 1.19  2001/06/18 20:17:43  oh
  Teames -> Teams

  Revision 1.18  2000/10/12 10:04:08  MH
  RFC/PPP: E-Mailadresse immer aus 'username@pointname.domain' bilden

  Revision 1.17  2000/10/10 07:54:11  rb
  Supersedes-Support fertiggestellt
  - Autoversand
  - SV: Einzelnachrichten

  Revision 1.16  2000/08/02 15:16:49  MH
  RFC/PPP: Reghinweis bei AM unterbinden

  Revision 1.15  2000/07/30 16:14:49  MH
  RFC/PGP: Jetzt auch im PGP-MenÅ

  Revision 1.14  2000/07/28 13:07:03  MH
  RFC/PPP: PGP vorbereitet

  Revision 1.13  2000/07/15 10:40:08  MH
  RFC/PPP: Online-Anruf in verÑnderter Form eingebunden
           (siehe Onlinehilfe)

  Revision 1.12  2000/06/30 19:45:17  MH
  RFC/PPP: Onlineanruf verboten...

  Revision 1.11  2000/06/10 11:54:37  MH
  RFC/PPP: Vertretersysteme nun moeglich

  Revision 1.10  2000/06/08 20:07:05  MH
  Teamname geandert

  Revision 1.9  2000/06/06 08:00:24  MH
  RFC/PPP: Weitere Anpassungen und Fixes

  Revision 1.8  2000/06/04 18:06:11  MH
  RFC/PPP: BoxNamenLen auf 8 Zeichen begrenzt

  Revision 1.7  2000/05/22 13:00:57  MH
  RFC/PPP: Umlaute im Realnamen erlaubt

  Revision 1.6  2000/05/17 17:26:15  MH
  Neuer Boxentyp: RFC/PPP

  Revision 1.5  2000/04/10 22:13:17  rb
  Code aufgerÑumt

  Revision 1.4  2000/04/09 18:32:39  openxp
