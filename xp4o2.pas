...
{ --------------------------------------------------------------- }
{ $Id: xp4o2.pas,v 1.16 2001/07/22 12:06:37 MH Exp $ }
...
uses  xpglobal, {$IFDEF virtualpascal}sysutils,{$endif}
      ...
      resource,help, xp0,xp1,xp1input,xpnt,xpcrc32;
...
begin
  ...
  if mp then
    if not dbPack(fname) then
      trfehler1(441,fname+dbExt,errortimeout_mini)     { 'Zu wenig Speicherplatz, um %s zu packen!' }
    else begin
...
begin
  ...
  if not exist(fn+dbExt) then
    trfehler1(440,fn+dbExt,errortimeout)    { 'XPack - unbekannte Datei: %s' }
  else begin
    closedatabases;
    packit(true,fn);
    opendatabases;
...
var hdp    : headerp;
    ...
    outofmem,
    outoflevels: boolean;
    nbetr  : string[40]; { BetrefflÑnge in der Datenbank }

  procedure RecurBez(ebene:shortint; rec:longint; spuren:komlines; last:boolean;
  ...
  var id     : longint;
    ...
    procedure wr;
    begin
      dbReadN(mbase,mb_betreff,nbetr);
      dbReadN(mbase,mb_brett,_brett);
      if nullid=0 then begin
        with kombaum^[komanz] do begin
          ...
          if recount(nbetr)=0 then;
          if left(nbetr,35)<>left(betr,35) then
            inc(flags,kflBetr);
  ...
  begin
    if (maxavail<4000) or not (komanz<realmaxkom) then outofmem:=true;
    if not (ebene<kommemax) then outoflevels:=true;
    if (ebene<kommemax) and not outofmem and
       (rec<>0) and not dbDeleted(mbase,rec) then begin
      if ebene>maxebene then inc(maxebene);
      if nullid=0 then
        dbGo(mbase,rec);
      wr;
      getmem(newbetr,length(nbetr)+1);
      fastmove(nbetr,newbetr^,length(nbetr)+1);
      GetSeekID;
      if dbFound then
        repeat
          ...
          for i:=1 to anz do begin
            ...
            if mmore and (ebene<kommemax-1) then begin
              spnr:=ebene div wordsize;
              spb:=ebene and (wordsize-1);
...
begin
  ...
  new(hdp);
  realmaxkom:=min(maxkomm,(memavail-8000) div 2 div sizeof(komrec));
  ...
  dispose(hdp);
  closebox;
  if outoflevels then rfehler(448);
  if outofmem then rfehler(449);
...
begin
  with kombaum^[bezpos] do begin
    ...
    if _ebene=0 then
      ...
      _ebene:=min(_ebene,kommemax);
      ss:=sp((_ebene-1)*komwidth);
      for i:=0 to _ebene-2 do begin
        sn:=i div wordsize;
        sb:=i and (wordsize-1);
...
{
  $Log: xp4o2.pas,v $
  Revision 1.16  2001/07/22 12:06:37  MH
  - FindFirst-Attribute ge‰ndert: 0 <--> DOS.Archive

  Revision 1.15  2001/04/04 19:57:02  oh
  -Timeouts konfigurierbar

  Revision 1.14  2000/11/10 19:52:06  rb
  Schînheitsfixes am Kommentarbaum

  Revision 1.13  2000/11/10 13:11:51  rb
  ...
  Revision 1.12  2000/11/09 23:48:13  rb
  Kommentarbaum Fixes

  Revision 1.11  2000/11/09 17:11:34  rb
  - Kommentarbaum-Limits geaendert: 65 Ebenen, 4680 Nachrichten
  - Kommentarbaum ist mit Crtl und Cursor rechts/links verschiebbar

  Revision 1.10  2000/11/07 20:21:17  rb
  Kommentarbaum Fixes

  Revision 1.9  2000/11/06 17:14:41  rb
  Kommentarbaum-Fix

  Revision 1.8  2000/11/02 01:11:40  rb
  Kommentarbaumlimits erhîht

  Revision 1.7  2000/09/30 11:47:23  MH
  - Fix: Bei Sprachwechsel wurde der Extracttyp nicht sauber Åbernommen

  Revision 1.6  2000/06/13 18:58:41  tg
  ...
  Revision 1.4  2000/04/09 18:25:31  openxp
