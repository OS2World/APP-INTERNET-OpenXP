...
{ --------------------------------------------------------------- }
{ $Id: keys.pas,v 1.7 2000/06/08 20:04:40 MH Exp $ }
...
{$IFDEF OS2 }
  kb_shift:=false;
  if (GetKeyboardState(kbdstf_RightShift)=true) or (GetKeyboardState(kbdstf_LeftShift)=true) then kb_shift:=true;
{$ENDIF }
end;
...
{$ENDIF }
{$IFDEF OS2 }
  kb_ctrl:=GetKeyboardState(kbdstf_Control);
{$ENDIF }
end;
...
{$ENDIF }
{$IFDEF OS2 }
  kb_alt:=GetKeyboardState(kbdstf_Alt);
{$ENDIF }
end;
...
{TJ 040600 - Tipp von dink@linnea.aso.edu.stockholm.se, um auch CTRL-P etc. auszulesen}
{$IFDEF OS2 }
procedure setkbd_binmode;
var
        kbd : kbdinfo;
...
{
  $Log: keys.pas,v $
  Revision 1.7  2000/06/08 20:04:40  MH
  Teamname geandert

  Revision 1.6  2000/06/04 14:32:28  tj
  SetKbd_BinMode Aufruf in XP.PAS ausgegliedert

  Revision 1.5  2000/06/04 11:50:52  tj
  unter OS/2 kann nun auch CTRL-P ausgelesen werden

  Revision 1.4  2000/04/09 18:09:35  openxp
