  ...
  begin
    with alist^ do
      if stat.statline then begin
        ...
        else if stat.helpinfo then write('   F1-',ListHelpStr); {!TG!, Spaces um 1 Byte gekÅrzt}
        mon;
        end;
    disp_DT;
  end;
...
begin
  with alist^ do begin
    ...
    if stat.statline then begin
      attrtxt(col.colstatus);
      mwrt(l,o,sp(w));
      {!TG!, Brettanzeige bei Uhr im Vollbildlister um 8 Zeichen nach links rÅcken}
...
end.
{
  ...
  Fix:
  - Absturz bei Nachrichten, deren Grî·e 0 Byte ist:
  ...
  Fix: Wurden markierte Zeilen in eine Datei Åbertragen,
       wurde ein Leerzeichen (#20) vor #13#10 eingefÅgt
