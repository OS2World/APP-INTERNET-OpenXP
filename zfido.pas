...
{ --------------------------------------------------------------- }
{ $Id: zfido.pas,v 1.18 2002/02/22 17:29:09 mm Exp $ }
...
uses  xpglobal,
      crt,dos,dosx,typeform,fileio,xpdiff,xpdatum,mimedec;
...
procedure ExpandCR(var data; bpos:word; size:word;
                   var addlfs:word); {&uses ebx, esi, edi} assembler;
asm
{$IFNDEF Ver32 }
       push ds
...
@ende: les    di,addlfs
       ...
       pop ds
{$ELSE}
       mov    edi,data          { edi -> msgbuf^[0] }
       mov    esi,data          { esi -> msgbuf^[bpos] }
       mov    ebx,bpos          { max. Anzahl einfÅgbarer LFs }
       add    esi,ebx
       mov    ecx,size          { ecx <- mbufsize-bpos }
...
procedure Remove0(var data; size:word); {&uses edi} assembler;
asm
{$IFNDEF Ver32 }
        les    di,data
        mov    cx,size
    ...
    repeat
      ...
      if line<>'' then begin
        ...
        if p=0 then ok:=false
          ...
          if id='MAILER' then programm:=left(line,120) else
...
var f1,f2   : file;
  ...
  begin
    ...
    with phd do begin
      ...
      DestNode:=_to.node;
      {$IFDEF Ver32 }
      getdate(year32,month32,day32,dummy32);
      dec(month32);
      gettime(hour32,min32,sec32,dummy32);
      year:=year32;
      month:=month32;
      day:=day32;
      dummy:=dummy32;
  ...
  begin
    ...
    with hd do begin
      ...
      if programm<>'' then
        wrs(^A'PID: XP '+mid(programm,cpos(' ',programm)+1));
      xflags:='';
  ...
  begin
    ...
    while (size>0) and not eof(f1) do begin
      re:=rr-re;
      blockread(f1,charrp(buf)^[re],min(size,bufsize-re),rr);
      rt:=rr;
      inc(rr,re);
      Remove0(buf^,rr);          { #0-Zeichen entfernen }
      re:=rr;
      if re<size then
        while (re>0) and (charrp(buf)^[re-1]<>#10) do dec(re);
      if re>0 then begin
        i:=0;
        repeat
          while (charrp(buf)^[i]<>#10) and (i<re) do inc(i);
          if (i<re) and (i>4) then
            if (charrp(buf)^[i-1]=#13) and (charrp(buf)^[i-2]=' ')
            and (charrp(buf)^[i-3]<>'-') and (charrp(buf)^[i-4]<>'-') then begin
              fastmove(charrp(buf)^[i+1],charrp(buf)^[i-2],rr-i-1);
              dec(re,3);
              dec(rr,3);
            end
            else inc(i)
      ...
      blockwrite(f2,buf^,re);
      if re>0 then
        lastchar:=charrp(buf)^[re-1];
      dec(size,rt);
      if re<rr then
        fastmove(charrp(buf)^[re],buf^,rr-re);
    end;
...
label abbr;
  ...
  function seek0(var buf; size:word):word; {&uses edi} assembler; { suche #0 }
  asm
{$IFNDEF Ver32 }
    mov cx, size
    les di, buf
...
{$ENDIF }
  ...
  { MK 06.02.2000 aus Inline in Asm konvertiert }
  function seekt(var buf; size:word):word; {&uses edi} assembler;  { suche _'---'_ }
  asm
{$IFNDEF Ver32 }
        mov cx, size
        les di, buf
...
{$ENDIF }
  ...
  { MK 06.02.2000 aus Inline in Asm konvertiert }
  procedure exch_8d(var buf; size:word); {&uses edi} assembler;
  asm
{$IFNDEF Ver32 }
        mov cx, size
        les di, buf
...
{
  $Log: zfido.pas,v $
  Revision 1.18  2002/02/22 17:29:09  mm
  - ISO2IBM durch ISO1toIBM aus Unit MIMEDEC ersetzt
  - MAC2IBM nach MIMEDEC verschoben

  Revision 1.17  2001/07/22 12:07:09  MH
  - FindFirst-Attribute ge‰ndert: 0 <--> DOS.Archive

  Revision 1.16  2001/06/18 20:17:44  oh
  Teames -> Teams

  Revision 1.15  2001/01/02 21:45:02  rb
  Softlinebreaks im Body outgoing entfernen

  Revision 1.14  2000/11/12 09:25:40  MH
  PID angepasst...

  Revision 1.13  2000/10/07 20:47:07  rb
  F-TO-énderung teilweise wieder zurÅckgenommen (ZFIDO, MAGGI)

  Revision 1.12  2000/10/07 00:55:19  rb
  Mailerstring auf 120 Zeichen verlÑngert

  Revision 1.11  2000/09/20 21:42:06  oh
  Patch 1 (Frank Ellert)

  Revision 1.10  2000/07/05 21:44:53  MH
  Mailerzeile:
  - auf 66 Zeichen (in/out) verlÑngert
  - mit (XP2) gekennzeichnet

  Revision 1.9  2000/06/08 20:08:19  MH
  Teamname geandert

  Revision 1.8  2000/05/28 21:18:31  MH
  Fido: Mailerzeile von 40 auf 60 Zeichen vergroessert

  Revision 1.7  2000/05/22 22:24:38  rb
  Assemblerteile nach 32 Bit portiert

  Revision 1.6  2000/05/17 20:10:26  tj
  ZFido ist mit VP 2.0 durchcompilierbar

  Revision 1.5  2000/04/10 22:13:18  rb
  Code aufgerÑumt

  Revision 1.4  2000/04/09 18:34:36  openxp
