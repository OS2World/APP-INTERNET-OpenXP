...
{ --------------------------------------------------------------- }
{ $Id: maggi.pas,v 1.20 2002/03/13 01:41:27 rb Exp $ }
...
{$IFDEF BP }
  xms,
{$ENDIF }
  typeform,fileio,montage,xpdatum,xp_iti,inout,mimedec;
...
type  charr       = array[0..65530] of char;
      ...
      header = record
                 ...
                 org_msgid  : string[midlen];   { X-XP-ORGMID }
                 org_xref   : string[midlen];   { X-XP-ORGREF }
      ...
      while ok and (s<>'') do begin
        ...
        if s[1]<>'$' then
          ...
          if (mc='$ MSG-ID  :') or (mc='$ MSGID   :') then
            msgid:=ff(copy(s,12,midlen)) else
          if (mc='$ REPLY-ID:') or (mc='$ X-REF   :') then
            ref:=ff(copy(s,12,midlen)) else
      ...
      repeat
        ...
        if (s<>'') and (s[1]<>'#') then begin
          ...
          case c of
            ...
            'Y' : gate:=left(s,80);
            { /fb }
            'I' : org_msgid:=left(s,midlen);
            'R' : org_xref:=left(s,midlen);
...
{
  $Log: maggi.pas,v $
  Revision 1.20  2002/03/13 01:41:27  rb
  ISO2IBM Parameter angepasst

  Revision 1.19  2002/02/19 15:40:37  mm
  lokale Prozedur ISO2IBM entfernt, dafuer Unit MIMEDEC eingebunden

  Revision 1.18  2001/06/18 20:17:18  oh
  Teames -> Teams

  Revision 1.17  2001/06/11 10:28:28  mm
  - BetreffLen auf 248 gesetzt
  - sicherheitshalber Strings in procedure wrs auf 253 Zeichen kÅrzen

  Revision 1.16  2001/06/10 11:26:31  mm
  BetrefflÑnge von 70 auf 250 Zeichen hochgesetzt

  Revision 1.15  2000/10/13 14:00:19  MH
  Replace:
  - Wir verabschieden uns von X-XP-MSG und setzen nun die Datenbank ein

  Revision 1.14  2000/10/11 21:25:00  MH
  XP-MSG: MyMail...hinzugefÅgt

  Revision 1.13  2000/10/07 20:47:06  rb
  F-TO-énderung teilweise wieder zurÅckgenommen (ZFIDO, MAGGI)

  Revision 1.12  2000/10/07 00:55:15  rb
  Mailerstring auf 120 Zeichen verlÑngert

  Revision 1.11  2000/09/20 21:42:04  oh
  Patch 1 (Frank Ellert)

  Revision 1.10  2000/09/07 15:01:50  MH
  XP˝ in XP2 umbenannt

  Revision 1.9  2000/09/07 14:59:22  MH
  Headereintrag: XPMODE fÅr HeaderOnly hinzugefÅgt

  Revision 1.8  2000/07/18 21:14:40  rb
  Supersedes - UUZ outgoing

  Revision 1.7  2000/07/05 21:44:53  MH
  Mailerzeile:
  - auf 66 Zeichen (in/out) verlÑngert
  - mit (XP2) gekennzeichnet

  Revision 1.6  2000/06/08 19:21:57  MH
  Teamname geaendert

  Revision 1.5  2000/04/10 22:13:05  rb
  Code aufgerÑumt

  Revision 1.4  2000/04/09 18:11:02  openxp
