...
var s     : string;
  ...
  begin
    ...
    if (list_markanz=0) and ((pos('@',s1)=0) or (s1='')) then begin
      fehler(getres(172)); { 'Keine gÅltige Message-ID markiert' }
      exit;
    end;
      ...
      if mid_ok and (pos('@',s1)<>0) then begin
        ...
        for i:=1 to filesize(reqfile) do begin
          ...
          if (midrec.aktiv<>0) and (midrec.mid=s1) then begin
            idfound:=true;
            { Der User wÅrde nicht erkennen, das einige Anforderungen
    ...
    close(reqfile);
    if ungueltig<>0 then fehler(getres(171)); { 'UngÅltige Message-ID erkannt' }
...
begin
  ...
  then hinweis(getres(136)) { 'Suchfunktion bei aktiver Liste markierter Nachrichten nicht mîglich!' }
  else begin
...
end.
{
  ...
  - auch CMD fÅr FileDa akzeptieren

  Revision 1.51  2001/06/22 18:18:54  MH
  - User-, Betreff- MsgID-Suche mu· wÑhrend Nachrichten bereits schon
  ...
  Bugfix: externer OS/2-Viewer lîscht Fileattach nicht mehr
  ...
  Message-ID Request nun Åber F3 zu erreichen
  ...
  Message-ID Request kann nun mehrere ID's Åbernehmen

  Revision 1.10  2000/08/18 14:59:47  MH
  RFC/PPP: Message-ID Request: Reihenfolge im MenÅ geÑndert
