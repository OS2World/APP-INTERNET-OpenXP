...
{ --------------------------------------------------------------- }
{ $Id: xp7f.pas,v 1.11 2001/07/22 12:06:50 MH Exp $ }
...
begin
  ...
  with BoxPar^ do begin
    ...
    findfirst(ImportDir+'*.*',DOS.Archive,sr);
    clrflag:=(doserror=0);
    ...
    if exist(XFerDir+'*.PKT') then begin
      ...
      window(1,1,80,25);
      if errorlevel<>0 then
        trfehler(719,errortimeout)   { 'fehlerhaftes Fido-Paket' }
      else begin
        if nDelPuffer then
          findfirst(XFerDir+'*.*',DOS.Archive,sr)
        else begin
          findfirst(XFerDir+'*.pkt',DOS.Archive,sr);    { .PKT - Dateien lîschen  }
...
begin
  ...
  window(1,1,80,25);
  findfirst(XFerDir+'*.*',DOS.Archive,sr);            { SPOOL leeren }
  while doserror=0 do begin
    UpString(sr.name);
...
{  if carrier(comnr) and not comn[comnr].IgCD then
  ...
  if (aresult<0) or (aresult>EL_max) then begin
    DropAllCarrier;
    trfehler1(720,strs(aresult),errortimeout_short);   { 'interner Fehler (%s) im Fido-Mailer' }
    aresult:=EL_break;
  end;
  ...
  case aresult of
    ...
    EL_carrier:begin
                 trfehler(721,errortimeout);   { 'Fehler - bitte Modemeinstellungen (Carrier) ÅberprÅfen!' }
                 goto fn_ende;
               end;
    EL_par   : begin
                 trfehler(722,errortimeout);   { 'Fehler beim XP-FM-Aufruf ?!' }
                 goto fn_ende;
               end;
...
{
  $Log: xp7f.pas,v $
  Revision 1.11  2001/07/22 12:06:50  MH
  - FindFirst-Attribute ge‰ndert: 0 <--> DOS.Archive

  Revision 1.10  2001/06/18 20:17:36  oh
  Teames -> Teams

  Revision 1.9  2001/04/04 19:57:02  oh
  -Timeouts konfigurierbar

  Revision 1.8  2000/12/31 11:43:11  MH
  Fix FidoNetcall-Anrufbericht:
  - Bei Fido (Crash) wurden falsche Boxparas verwendet

  Revision 1.7  2000/12/25 17:13:49  MH
  Fido: Fix:
  - Netzanrufbericht enthielt nicht korekten Betreff

  Revision 1.6  2000/12/09 16:37:40  MH
  - vor schlie·en flushen

  Revision 1.5  2000/04/10 22:13:12  rb
  Code aufgerÑumt

  Revision 1.4  2000/04/09 18:27:32  openxp
