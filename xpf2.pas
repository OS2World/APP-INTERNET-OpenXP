...
{ --------------------------------------------------------------- }
{ $Id: xpf2.pas,v 1.7 2001/07/22 12:07:00 MH Exp $ }
...
uses  xpglobal, {$IFDEF virtualpascal}sysutils,{$endif}
      crt,dos,typeform,fileio,archive,montage,
...
{ TIC-Files verarbeiten; BoxPar^ mu· korrekte geladen sein! }
...
begin
  ...
  while not eof(t) do begin        { gepackte TIC-Files auswerten }
    ...
    if (s[1]='*') and (pos('  rcvd ',lstr(s))>0) then begin
      ...
      then begin   { mîgliches TIC-Paket? }
        at:=ArcType(s);
        if at<>0 then begin
          OpenArchive(s,at,ar);
          ...
          if not ar.ende then begin           { .TIC-Files enthalten }
            ...
            if UniExtract(s,FilePath+'TICK\','*.*') and
              ...
              _era(s);
              FindFirst(FilePath+'TICK\*.TIC',DOS.Archive,sr);
...
ende:
  ...
  findfirst(FilePath+'*.TIC',DOS.Archive,sr);    { ungepackte TIC-Files }
...
{
  $Log: xpf2.pas,v $
  Revision 1.7  2001/07/22 12:07:00  MH
  - FindFirst-Attribute ge‰ndert: 0 <--> DOS.Archive

  Revision 1.6  2000/05/25 23:26:29  rb
