...
{ --------------------------------------------------------------- }
{ $Id: xp5.pas,v 1.12 2001/07/22 21:20:21 MH Exp $ }

{ CrossPoint - Utilities }

{ TJ 010600 - Es wurden die Funktionen MON/MOFF unter OS/2 deaktiviert, }
{             sowie SoftSave auf FALSE gesetzt                          }
...
procedure XpUpdateWelcome;
function EnterPGPPassphrase:string;
...
{$ENDIF}
{$IFDEF OS2 }
   move(CopyScr2,OS2ScrPtr2^,OS2ScrLen2);
   VioShowBuf(0,OS2ScrLen2,0);
{$ENDIF }
  end;
...
{$ENDIF}
{$IFDEF OS2 }
    VioGetBuf(OS2ScrPtr2,OS2ScrLen2,0);
    SelToFlat(OS2ScrPtr2);
    move(OS2ScrPtr2^,CopyScr2,OS2ScrLen2);
    mattr:=textattr;
    textbackground(black);
    textcolor(lightgray);
    cursor(curoff);
...
{$IFDEF BP }
  SetXPborder;
{$ENDIF }
{$IFDEF WIN32} { nicht schîn, aber besser als nix }
  showscreen(false);
  showlastkeys;
  aufbau:=true;
{$ENDIF}
...
begin
  ...
  if ReadFilename(getres(503),fn,true,useclip) then begin   { 'Bildschirm-Auszug' }
    ...
    for y:=1 to screenlines do begin
      line:='';
      for x:=1 to 80 do
        line:=line+copychr(x,y);
      writeln(t,Rtrim(line));
    end;
    message('OK.');
    close(t);
...
var x,y : byte;
  ...
  begin
    maus_gettext(xx,yy);
    if (xx<x) or (xx>x+26+length(u)) or (yy<y) or (yy>y+4) then
      if (t=mausunleft) then
        t:=keycr
      else if (t=mausunright) then
        t:=keyesc;
...
begin
  u:=getres2(3000,4);
  diabox(27+19+length(u),5,'',x,y);
  mwrt(x+3,y+2,u+':');
  attrtxt(col.coldiainp);
  mwrt(x+6+length(u),y+2,sp(18+19));
  s:='';
  brk:=false;
  repeat
    attrtxt(col.coldiainp);
    mwrt(x+7+length(u),y+2,dup(length(s),'*')+sp(16+19-length(s)));
    gotoxy(x+7+length(u+s),y+2);
    get(t,curon);
    if (t>=mausfirstkey) and (t<=mauslastkey) then
      maus_bearbeiten;
    if (t=keyesc) then brk:=true
  ...
  closebox;
  freeres;
  EnterPGPPassphrase:=iifs(brk or (s=''),'',s);
end;
...
begin
  ...
  moff;
  attrtxt(col.colmboxhigh);
  wrt(x+3,y+2,reps(getreps2(511,1,xp_xp), verstr));  { 'Willkommen bei CrossPoint v3.30.001' }
  attrtxt(col.colmbox);
  for i:=2 to anz do
...
{
  $Log: xp5.pas,v $
  Revision 1.12  2001/07/22 21:20:21  MH
  - wenigstens ein bisschen ScreeSaven

  Revision 1.11  2001/07/22 12:06:39  MH
  - FindFirst-Attribute ge‰ndert: 0 <--> DOS.Archive

  Revision 1.10  2001/07/03 18:05:58  MH
  - pophp-Aufruf verschoben

  Revision 1.9  2001/06/18 20:17:32  oh
  Teames -> Teams

  Revision 1.8  2001/01/16 18:14:53  MH
  Fixes fÅr Passwortabfrage:
  - Absturz bei Eingabe von zu vielen Zeichen

  Revision 1.7  2000/10/12 21:44:43  oh
  -PGP-Passphrase merken + Screenshot-File-Verkleinerung

  Revision 1.6  2000/06/24 16:46:10  tg
  hart codiertes Update auf v3.2 auf Verstr geaendert

  Revision 1.5  2000/06/01 15:01:15  tj
  Bildschirmschoner fuer OS/2 portiert, sowie Sternen-Bildschirmschoner fuer mehr als 25 Zeilen angepasst

  Revision 1.4  2000/04/09 18:26:01  openxp
