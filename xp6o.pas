...
{ --------------------------------------------------------------- }
{ $Id: xp6o.pas,v 1.25 2001/12/30 12:01:55 mm Exp $ }
{.$DEFINE ARCTEST}
{$I XPDEFINE.INC}
{$IFDEF BP }
  {$O+,F+}
...
uses xpglobal,
  ...
  crt,dos,typeform,fileio,inout,keys,datadef,database,maske,
...
begin
  ...
  findfirst(ownpath+iifs(crash,'*.cp','*.pp'),DOS.Archive,sr);
  found:=false;
...
{$IFDEF ARCTEST}
{ ARCTEST: Hier wird das Original verschoben...Header bleibt vollstÑndig
  erhalten...MID wird rotiert - bitte noch nicht verwenden!

  Notwendige énderungen: Original nur kopieren; Markierte Liste beachten }

procedure MsgArchivieren(newbrett:string);
var x,y     : byte;
    brk     : boolean;
...
begin
  ...
  if size>0 then begin      { mÅ·te eigentlich immer TRUE sein }
    ...
    with hdp^ do
    if msgid<>'' then
      msgid:=right(msgid,1)+left(msgid,length(msgid)-1);
    fn:=TempS(hds);
    assign(f1,fn);
    rewrite(f1,1);
...
begin
  ...
  if typ=7 then
   ...
   {$IFDEF ARCTEST}
   if typ=5 then recbb:=dbRecno(bbase);
   {$ENDIF}
...
{ Brettvertreter  }    if not pm then begin
                         ...
                         if (ntBoxNetztyp(pollbox) in [nt_PPP,nt_UUCP,nt_ZConnect]) then begin
                           Am_ReplyTo:=empf;
                   ...
                   if typ=3 then begin
                     ...
                     if (hdp^.netztyp in [nt_UUCP,nt_PPP]) then begin
               ...
               {$IFDEF ARCTEST}
               if typ=5 then MsgArchivieren(empf)
               {$ELSE}
               ...
               {$ENDIF}
                 ...
                 if typ in [1,7] then begin
                   ...
                   { wÑre es sinnvoll, die BezÅge zu retten?!?
                   _orgref:=hdp^.org_msgid;
                   _beznet:=hdp^.netztyp;
                   AddToReflist(hdp^.ref);
                   _ref6list:=reflist;
                 ...
                 { Ellert-Fix auskommentiert - s. MY!
                 if typ=7 then begin
                   sData^.oab:=hdp^.absender;
                   sData^.oar:=hdp^.realname;
                   sData^.orghdp:=hdp;
         ...
         6 : begin
               ...
               if not dbFound then begin   { User noch nicht vorhanden }
                 ...
                 b:=1+iif(newuseribm,0,8);        { Umlaute   }
               ...
               wrkilled;
               aufbau:=true; xaufbau:=true;
               setbrettgelesen(obrett);  { evtl. Ungelesen-Flag im Originalbrett lîschen }
             end;
  end;  { case }
...
{
  $Log: xp6o.pas,v $
  Revision 1.25  2001/12/30 12:01:55  mm
  - Archivierungsvermerk abschaltbar unter:
    Config/Optionen/Allgemeines/"Archivierungsvermerk erstellen"
    (Jochen Gehring)

  Revision 1.24  2001/07/22 12:06:41  MH
  - FindFirst-Attribute ge‰ndert: 0 <--> DOS.Archive

  Revision 1.23  2001/07/17 09:06:24  MH
  - Ellert-Fixpatch entfernt! (MY)
  - Zeigt ein PM-Qoute auf ein AM-Brett, dann diesen zulassen

  Revision 1.22  2001/06/18 20:17:33  oh
  Teames -> Teams

  Revision 1.21  2000/12/30 00:51:22  MH
  Fix:
  - N/W/K/D erzeugte eine defekte 'Diskussion-in:' Headerzeile

  Revision 1.20  2000/12/26 13:54:39  MH
  -.-

  Revision 1.19  2000/12/25 17:13:38  MH
  - NewUserIBM bei Useraufnahme ergÑnzt
  - Fix: Archivierung von PMs (Alt+P)

  Revision 1.18  2000/12/05 21:21:17  rb
  'Tearline verfremden' Åberarbeitet

  Revision 1.17  2000/11/17 16:47:48  rb
  Fix: Kommentarbaum wurde manchmal unnîtigerweise neu aufgebaut

  Revision 1.16  2000/11/01 03:24:03  rb
  Fido: Tearline+Origin bei Nachricht/Weiterleiten/Kopie&EditTo verfremden

  Revision 1.15  2000/10/13 20:07:00  MH
  Followup-To for PMs only...

  Revision 1.14  2000/10/10 18:55:49  rb
  Supersedes-Support fertiggestellt
  - Autoversand
  - SV: Einzelnachrichten

  Revision 1.13  2000/09/20 21:47:25  oh
  Patch 2 (Frank Ellert)

  Revision 1.12  2000/08/10 17:31:35  MH
  JG: SetBrettGelesen-Routine fÅr Ungelesen-Bug

  Revision 1.11  2000/07/21 10:33:50  MH
  Fix: Weiterleiten/EmpfÑnger Ñndern
     - Bei vorhandenem Origin wenigstens Schreibsperre beachten

  Revision 1.10  2000/07/21 09:29:01  MH
  Fix: Weiterleiten/EmpfÑnger Ñndern
     - Wurde einem Fidobrett ein Origin zugewiesen, wurde dieser als
       EmpfÑnger eingetragen...:-/

  Revision 1.9  2000/07/18 00:02:29  MH
  Weiterleiten: Meldung fÅr Schreibzugriff-Sperre geÑndert

  Revision 1.8  2000/07/09 16:26:13  MH
  Weiterleiten unter Beachtung von Vertreter und Schreibsperre:
  - Nochmals nachgebessert

  Revision 1.7  2000/07/09 15:24:10  MH
  - Weiterleiten: Ein wenig nachgebessert

  Revision 1.6  2000/07/09 11:06:41  MH
  Weiterleiten bei Brettern mit Vertretern und Schreibsperre
  findet nun Beachtung

  Revision 1.5  2000/06/02 09:46:37  MH
  RFC/PPP: Weitere Anpassungen

  Revision 1.4  2000/05/25 23:26:27  rb
